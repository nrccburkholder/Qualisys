CREATE PROCEDURE dbo.sp_SI_CommentBox2
    @QuestionFormID int,
    @intDODSurvey   int
AS
...
SELECT CLP.QuestionForm_ID,
       ...
       convert(int, convert(varchar, MIN(CP.Y_Pos)) + convert(varchar, CLP.intPage_Num) + convert(varchar, CLP.CmntBox_id)) as Identifier
FROM CommentPos CP,
     CommentLinePos CLP
