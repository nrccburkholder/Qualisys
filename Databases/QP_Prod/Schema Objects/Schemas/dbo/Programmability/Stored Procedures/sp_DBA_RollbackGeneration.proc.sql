CREATE PROCEDURE sp_DBA_RollbackGeneration @Survey_id INT, @CurrentMailingStep INT=NULL, @intSequence INT=NULL,  @datGenerated DATETIME=NULL  ,  @datMailed DATETIME=NULL, @ScheduledToGenerate DATETIME=NULL          
AS          
/*          
...
11/26/07 MWB: Modified this procedure to hand in a optional MailDate parameter.        
This overrides the rule that generations cannot be rolled back if a mail date has been set.        
Make sure to run the listing of all generations (by sending in only the survey_ID) to make sure there was only 1 step        
on the given maildate if you indend on overriding a mail date.        
        
4/1/09 MWB: Modified Proc to include any possible records that may not         
have generated the first time and have a sentmail_ID of -1 in the scheduledmailing table.        
This can happen b/c of TOCL or other reasons and b/c of this we have no record in the sentmail table.          
We still want to provide the option to reset these in the scheduledmailing table.        
...
11/11/09 MWB: Added extra messages around VendorFileCreationQueue deletes to ensure records are being reset properly.    
...
if @datMailed is null        
 ...
 begin        
  ...
  FROM SentMailing sm, ScheduledMailing schm          
  WHERE sm.scheduledmailing_Id=schm.scheduledmailing_Id          
  AND CONVERT(VARCHAR(10),sm.datMailed,120)= @datMailed          
  AND CONVERT(VARCHAR(10),datGenerated,120)=@datGenerated           
  AND schm.MailingStep_id=@CurrentMailingStep          
 end        
...
if @ScheduledToGenerate is not null        
 begin        
  ...
  FROM ScheduledMailing schm, mailingmethodology mm        
...
PRINT 'DELETE next ScheduledMailing with -1 Sentmail_IDs'         
 DELETE ScheduledMailing          
  FROM ScheduledMailing schm          
  WHERE  MailingStep_id=@nextMailingStep AND            
  samplepop_id in (SELECT samplepop_id FROM #bad_SchedMail)          
