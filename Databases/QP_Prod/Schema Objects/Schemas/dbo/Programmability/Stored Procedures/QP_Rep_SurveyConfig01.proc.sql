CREATE PROCEDURE [dbo].[QP_Rep_SurveyConfig01]    
...
IF EXISTS (  
    ...
    UNION SELECT 'Survey Configuration' AS SheetNameDummy, 7 AS dummyPropNo, 'Sampling type' AS Property, CASE WHEN SamplingAlgorithmId=1 THEN 'Static' WHEN SamplingAlgorithmId=2 THEN 'Dynamic' WHEN SamplingAlgorithmId=3 THEN 'StaticPlus' END AS [Value] FROM Survey_def WHERE Survey_id=@intSurvey_id    
	...
	UNION SELECT 'Survey Configuration' AS SheetNameDummy, 10 AS dummyPropNo, 'ReSurvey Exclusion' AS Property, CONVERT(VARCHAR,INTRESurvey_PERIOD)+' months' AS [Value] FROM Survey_def WHERE Survey_id=@intSurvey_id    
