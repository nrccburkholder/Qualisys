CREATE PROCEDURE SP_FG_PopulateSkipPatterns @Survey_id INT, @GetDate DATETIME
AS
...
--------------------------------------------------------------------------------------------------------------------------------
-- Error Check  -- ccaouette 2/13/2014
--------------------------------------------------------------------------------------------------------------------------------
 with temp1 as
 (
	 SELECT DISTINCT @skip_id as skip_id, t.QstnCore
	 FROM #t t
	 WHERE t.qstnorder>@beginqstnorder
	 AND t.qstnorder<@endqstnorder
		AND t.subType=1
), temp2 as
(
	select skip_id, QstnCore, count(*) as NumofRecords
	from temp1 
	group by skip_id, QstnCore having count(*) > 1
)

 INSERT INTO SkipQstnsDups 
 (survey_id, skip_id, QstnCore, datGenerated, CreateDate, [ErrorMsg])
 SELECT DISTINCT @survey_id, t.skip_id, t.QstnCore, @GetDate, getdate(), 'Duplicate records exist in temp table. Records: '  + CAST(t.NumofRecords as varchar(100))
 FROM temp2 t

 INSERT INTO SkipQstnsDups 
 (survey_id, skip_id, QstnCore, datGenerated, CreateDate, [ErrorMsg])
 SELECT DISTINCT @survey_id, @skip_id, t.QstnCore, @GetDate, getdate(), 'Record already exists in the SkipQstns table'
 FROM #t t
 INNER JOIN SkipQstns sq on t.QSTNCORE = sq.QstnCore and sq.Skip_id = @skip_id
 WHERE t.qstnorder>@beginqstnorder
 AND t.qstnorder<@endqstnorder
    AND t.subType=1
--------------------------------------------------------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------------------------------------------------------
 ...
 SELECT DISTINCT @skip_id, t.QstnCore
 FROM #t t
 LEFT JOIN SkipQstns sq on t.QSTNCORE = sq.QstnCore and sq.Skip_id = @skip_id
 WHERE t.qstnorder>@beginqstnorder
 AND t.qstnorder<@endqstnorder
    AND t.subType=1
	AND sq.Skip_id IS NULL
