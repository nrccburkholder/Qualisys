--sp_helptext NQLValidationTest_skd_ch  
    
CREATE proc NQLValidationTest_skd_ch    
as                
set nocount on       
...
print'======================================================'              
if exists(select COUNT(distinct LITHO) as TotalCount from nqlvalcheck_SFH_Ch nql inner join SENTMAILING sm on nql.LITHO = sm.STRLITHOCODE)              
begin              
select COUNT(distinct LITHO) as TotalCount from nqlvalcheck_SFH_Ch nql inner join SENTMAILING sm on nql.LITHO = sm.STRLITHOCODE               
end              
...
print'======================================================'              
if exists(select LITHO from nqlvalcheck_SFH_Ch nql inner join SENTMAILING sm on nql.LITHO = sm.STRLITHOCODE              
where sm.DATMAILED is NULL)              
begin              
print 'All the below Lithos do not have Mail Date set'              
select distinct LITHO,sm.DATMAILED from nqlvalcheck_SFH_Ch nql inner join SENTMAILING sm on nql.LITHO = sm.STRLITHOCODE              
where sm.DATMAILED is NULL              
end else               
...
begin               
select LITHO, CONVERT(VARCHAR(10), GETDATE(), 101) as [Date of Exp] from nqlvalcheck_SFH_Ch nql inner join SENTMAILING sm on nql.LITHO = sm.STRLITHOCODE              
where datExpire <= GETDATE()              
end else               
...
if exists(              
select distinct LITHO from nqlvalcheck_SFH_Ch nql inner join SENTMAILING sm on nql.LITHO = sm.STRLITHOCODE              
...
print'======================================================              
'              
select distinct LITHO, qf.datResultsImported as DateImported from nqlvalcheck_SFH_Ch nql inner join SENTMAILING sm on nql.LITHO = sm.STRLITHOCODE              
inner join QUESTIONFORM qf on sm.SENTMAIL_ID = qf.SENTMAIL_ID               
inner join QUESTIONRESULT qr on qf.QUESTIONFORM_ID = qr.QUESTIONRESULT_ID              
where nql.LITHO = sm.STRLITHOCODE and sm.SENTMAIL_ID = qf.SENTMAIL_ID and qf.datResultsImported is NULL              
end else              
...
select distinct sm.STRLITHOCODE, qr.QSTNCORE,qr.INTRESPONSEVAL into #temp_1 from nqlvalcheck_SFH_Ch nql inner join SENTMAILING sm on nql.LITHO = sm.STRLITHOCODE          
inner join QUESTIONFORM qf on sm.SENTMAIL_ID = qf.SENTMAIL_ID           
...
where TABLE_NAME = 'nqlvalcheck_SFH_Ch' and COLUMN_NAME like 'Q%'           
...
exec('insert into tmp_skd_loop_qstncore select Litho, Q' + @qc + ','+''''+@qc+''''+' as Question from nqlvalcheck_SFH_Ch')          
          
--print('select Litho, Q' + @qc + ','+''''+@qc+''''+' as Question from nqlvalcheck_SFH_Ch')          
...
' case when INTRESPONSEVAL = 1 then cast(qr.QSTNCORE as varchar)+'+''''+'A'+'''' +           
  ...
  ' end as Question from nqlvalcheck_SFH_Ch nql inner join SENTMAILING sm on nql.LITHO = sm.STRLITHOCODE          
inner join QUESTIONFORM qf on sm.SENTMAIL_ID = qf.SENTMAIL_ID           
inner join QUESTIONRESULT qr on qf.QUESTIONFORM_ID = qr.QUESTIONFORM_ID           
...
exec( 'insert into tmp_skd_loop_qstncore_s select distinct sm.STRLITHOCODE, qr.QSTNCORE, qr.INTRESPONSEVAL from nqlvalcheck_SFH_Ch nql inner join SENTMAILING sm on nql.LITHO = sm.STRLITHOCODE          
inner join QUESTIONFORM qf on sm.SENTMAIL_ID = qf.SENTMAIL_ID           
inner join QUESTIONRESULT qr on qf.QUESTIONFORM_ID = qr.QUESTIONFORM_ID           
