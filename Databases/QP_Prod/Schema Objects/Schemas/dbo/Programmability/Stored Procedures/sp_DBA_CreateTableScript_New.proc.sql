CREATE proc sp_DBA_CreateTableScript_New @oName varchar(100), @newName varchar(100) = null
as 
...
select @cState = @newName +'|'+char(13) 
while @@fetch_status = 0
   begin
	select @cState = rtrim(@cState) +  
			 '|'+@colName + case  
				when len(rtrim(@colName)) < 8  then ' '
				when len(rtrim(@colName)) > 8 and len(rtrim(@colName)) < 16 then ' '
				else '' 
				end
				+ '|'+@datType 
				+ case 
