...
BEGIN
    SELECT vq.VendorFile_ID, vq.SampleSet_ID, cv.strClient_Nm, cv.Client_ID, cv.strStudy_Nm, cv.Study_ID, cv.strSurvey_Nm, cv.Survey_ID,
           ...
           CONVERT(VARCHAR, vq.DateDataCreated, 101) + ' ' + ms.strMailingStep_Nm AS Name, vq.ErrorDesc, ms.Vendor_ID
    ...
    WHERE cv.Survey_ID = ss.Survey_ID 
      ...
      AND CONVERT(VARCHAR, vq.DateDataCreated, 101) >= ISNULL(@StartDate, '1900-1-1')
      AND CONVERT(VARCHAR, vq.DateDataCreated, 101) <= ISNULL(@EndDate, '2149-1-1') 
    ORDER BY cv.strClient_Nm, cv.strStudy_Nm, cv.strSurvey_Nm, CONVERT(VARCHAR, vq.DateDataCreated, 101) + ' ' + ms.strMailingStep_Nm
END
ELSE
BEGIN
    SELECT vq.VendorFile_ID, vq.SampleSet_ID, cv.strClient_Nm, cv.Client_ID, cv.strStudy_Nm, cv.Study_ID, cv.strSurvey_Nm, cv.Survey_ID,
