CREATE PROCEDURE SP_Phase4_Big_View_Web @sampleset INT, @Cleansed BIT=0          
AS            
...
--  Modified 2/12/2010 MWB -- Had to split @sqlstr into @sqlStr1 and @sqlstr2 b/c study hit over 8000 char limit   
--  Modified 12/13/2010 DRM -- Added @sel2 due to 8000 char limit
      
DECLARE @strsql VARCHAR(8000), @strsql1 VARCHAR(8000), @strsql2 VARCHAR(8000)  
DECLARE @sql VARCHAR(1000), @ReportDate VARCHAR(50), @survey INT, @study INT, @fld VARCHAR(70), @short VARCHAR(70),            
...
print '@survey_ID = ' + cast(@survey as varchar(100))    
print '@MethodologyType = ' + cast(@MethodologyType as varchar(100))    
...
select @STRSQL1 = 'CREATE VIEW S' + CONVERT(VARCHAR,@study) + '.BIG_VIEW_WEB AS ' +            
  ...
  'CONVERT(VARCHAR(30),'''+@MethodologyType+''') MethodologyType ' --+ @sel +  commented out as it will be added later in EXEC statement  
           
  
select @strsql2 =' FROM selectedsample ss(NOLOCK), s' + CONVERT(VARCHAR,@study) + '.big_view bv(NOLOCK), samplepop sp(NOLOCK), sampleset s(NOLOCK) ' +             
