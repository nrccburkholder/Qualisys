CREATE procedure sp_phase3_comments_email_orig
as
...
/* Modified 2/5/2 BD This will need to be added back in once we want the emails to go to the clients. */
        ...
        IF @CCopy = @Recip
            SET @Subject = 'Service Alert - No Client Contact Specified Survey ' + convert(varchar,@surveyid)
        ELSE
            SET @Subject = 'Service Alert'
/*
        IF @CCopy = @Recip
            SET @Subject = 'Service Alert - for ADs only Survey ' + convert(varchar,@surveyid)
        ELSE
            SET @Subject = 'Service Alert - for ADs only Survey ' + convert(varchar,@surveyid)
*/        
        ...
        IF @cnt = 1
          BEGIN
          --Insert the top line
             INSERT INTO ##Email ( strLine ) VALUES ( 'This email has been sent as notification that ' + CONVERT(VARCHAR,@cnt) + ' Service Alert' )
             INSERT INTO ##Email ( strLine ) VALUES ( 'has been posted to your IDEAS web site.' )
          END

        ELSE
          BEGIN
          --Insert the top line
             INSERT INTO ##Email ( strLine ) VALUES ( 'This email has been sent as notification that ' + CONVERT(VARCHAR,@cnt) + ' Service Alerts' )
             INSERT INTO ##Email ( strLine ) VALUES ( 'have been posted to your IDEAS web site.' )
          END


/*-- TO BE TAKEN OUT WHEN ABOVE IS ADDED        
        --Insert the top line
...
*/        
        ...
        SET @strSql = 'master..xp_sendmail @recipients = ''' + @Recip + ''', ' + char(10) +
                      ...
                      '                    @blind_copy_recipients = ''JFleming; BDohmen'', ' + char(10) +
