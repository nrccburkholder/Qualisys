...
--  with flag set
-- MODIFIED: MB 3/4/09  added @bitUpdateDatamart flag.
--  We use this when we want to call the qualisys reset and datamart reset
--  at seperate times.  this should only be false when run as a proc by
--  a DBA not used by a program or application
-- MODIFIED: DBG 2/26/2013 added deletion from questionform_extract table
-- =====================================================================
...
DECLARE @InLitho VARCHAR(7700), @SQL VARCHAR(8000), @SERVER VARCHAR(50), @NonDel_dt DATETIME, @TimeStamp DATETIME
...
print 'Update the QuestionResult2 table'
 ...
 WHERE qr.QuestionForm_id = lt.QuestionForm_id
--------------------------------------------------------------------------------------------
 --Update the questionform_extract table
print 'Update the questionform_extract table'
 DELETE qfe
 FROM questionform_extract qfe, #Lithos lt
 WHERE qfe.QuestionForm_id = lt.QuestionForm_id
--------------------------------------------------------------------------------------------
