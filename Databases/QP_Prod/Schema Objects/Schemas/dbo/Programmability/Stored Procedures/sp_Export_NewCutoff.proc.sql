-- MODIFIED 2/6/4 BD removal of the unikeys table.
-- MODIFIED 4/8/4 BD Only accept returns processed prior to today.
--			That way the result will be in the datamart.
CREATE PROCEDURE sp_Export_NewCutoff
 @Survey_id INTEGER, 
 @Start_dt DATETIME, 
 @Stop_dt DATETIME, 
...
--We want the maximum date to be 11:59pm of yesterday.
IF @Stop_dt>DATEADD(DAY,-1,CONVERT(DATETIME,CONVERT(VARCHAR(10),GETDATE(),120)))
SELECT @Stop_dt=CONVERT(DATETIME,(CONVERT(VARCHAR(10),DATEADD(DAY,-1,GETDATE()),120)+' 23:59:59'))
 ...
 BEGIN
  update qf
    ...
    WHERE qf.Survey_id=@Survey_id
      ...
      AND qf.datResultsImported<CONVERT(DATETIME,CONVERT(VARCHAR(10),GETDATE(),120))  --midnight last night.
 ...
 BEGIN
  UPDATE QuestionForm
    ...
    WHERE Survey_id=@Survey_id
      ...
      AND datResultsImported<@Stop_dt
  IF @@ERROR <> 0
    BEGIN
      ROLLBACK TRANSACTION
      RETURN -1
...
BEGIN
    ...
    'UPDATE qf '+
      ...
      '  AND qf.Cutoff_id IS NULL'+
      '  AND qf.datResultsImported<CONVERT(DATETIME,CONVERT(VARCHAR(10),GETDATE(),120))'+
      '  AND qf.SamplePop_id=sp.SamplePop_id'+
