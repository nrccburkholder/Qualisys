/****** Object:  Stored Procedure dbo.SP_LoadedDataStats    Script Date: 6/9/99 4:36:36 PM ******/
/****** Object:  Stored Procedure dbo.SP_LoadedDataStats    Script Date: 3/12/99 4:16:07 PM ******/
/*************************************************************
   This procedure calculates stats on  the load tables 
   with the invalidation rules applied for each metafield type. 
   RC - 020799
   Revised by DPA on 03/01/99 - Previous Version: OldSP_LoadedDataStats
*************************************************************/
CREATE PROCEDURE SP_LoadedDataStats @study_id NUMERIC
AS
BEGIN
   SET NOCOUNT ON
...
*********************************************************************/
   ...
   BEGIN
      SELECT @workingString = 
                'SELECT ' + @field_nm + ' = COUNT(*) ' +
                  ' FROM S' + CONVERT(VARCHAR, @study_id) + '.' + @table_nm + '_load' +
