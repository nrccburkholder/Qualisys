CREATE    PROCEDURE SP_DBA_HCAHPSUpdate @Study_ID int, @DataFile_id int
AS
...
-- SET @SQL = 'SELECT ''' + @LTime + ''' = datReceived from ' + @SERVER + '.QP_Load.dbo.datafile WHERE DataFile_id = @DataFile_ID'
-- EXEC(@SQL)
...
CREATE TABLE #SPs (SampleUnit_ID int, SamplePop_ID int, Enc_ID int, DaysFromFirst int, 
				ReportDate DateTime, BigTableName varchar(100), SentMail_id int)

--Get all enc_id with SampleUnit.bitHCAHPS = 1
INSERT #SPs (SampleUnit_ID, SamplePop_ID, Enc_ID, ReportDate)
	SELECT su.SampleUnit_ID, sp.SamplePop_ID, ss.Enc_ID, ss.ReportDate 
