--exec SSRS_CE_SurveyExceptions '1/6/2008','1/12/2008 12:59:00.000','C253',1  
Create procedure [dbo].[SSRS_CE_GetSurveyTracking] @mindate datetime,@maxdate datetime,@selections varchar(8000),@bitExceptionsOnly bit  
as  
  
/*
declare @mindate datetime,@maxdate datetime,@selections varchar(8000),  @bitExceptionsOnly bit
  
set @mindate='1/6/2008'  
set @maxdate='1/12/2008'  
set @selections='C63'  
set @bitExceptionsOnly = 1
*/
...
set @maxdate=dateadd(ms,-3,dateadd(dd,1,dateadd(dd,datediff(dd,0,@maxdate),0)))  
...
if @bitExceptionsOnly=0  
begin  
set @criteria=@criteria  
end  
  
if @bitExceptionsOnly=1  
begin  
set @criteria=@criteria+ + ' and nsp.PeriodExpSamps-m.samplesmailed<>0'  
end  
  
print @criteria  
  
  
--create temp table with 'active' surveys  
create table #surveys (survey_id int)  
 create clustered index idx_clustered_surveyid on #surveys (survey_id)  
...
create table #NextSamplesPeriod (survey_id int,perioddef_id int,datNextSample datetime,[Period Name] varchar(100),[Sampling Freq] varchar(20),PeriodExpSamps float)  
...
create table #samples (survey_id int,[Total Samples] int,[Oversamples] int,[Samples Rolled Back] int,[Samples Scheduled] int,[LastSampEncDateRange] varchar(100))  
...
--populate surveys with 'active' surveys  
insert #surveys (survey_id)  
select distinct p.survey_id    
from perioddef p (nolock)    
   left outer join perioddates pd (nolock)    
    on (p.perioddef_id=pd.perioddef_id)    
where pd.datscheduledsample_dt between dateadd(qq,-1,getdate()) and dateadd(qq,1,getdate())  
...
from perioddef p (nolock)  
 right outer join perioddates pd (nolock)  
  on (p.perioddef_id=pd.perioddef_id)  
where p.survey_id in (select survey_id from #surveys)  
...
set [Period Name]=p.strPerioddef_nm,  
 ...
 [PeriodExpSamps]=round(@intdategap/(datediff(dd,p.datExpectedEncStart,p.datExpectedEncEnd)/(p.intexpectedsamples*1.0)),0)  
...
insert #samples (survey_id,[Total Samples],[Oversamples],[Samples Rolled Back],[Samples Scheduled],[LastSampEncDateRange])  
select surv.survey_id, s.samples+isnull(r.rollbacks,0),o.oversamples,isnull(r.rollbacks,0), sch.Scheduled,dr.[LastSampEncDateRange]  
from   
 #surveys surv  
 left outer join   
 (select survey_id,  
 ...
 from sampleset  
 where datsamplecreate_dt between @mindate and @maxdate and tiOversample_flag=0  
 group by survey_id) s  
  on (surv.survey_id=s.survey_id)  
 left outer join   
 (select survey_id,  
  sum(case when sampleset_id is null then 0 else 1 end) as oversamples  
 from sampleset  
 where datsamplecreate_dt between @mindate and @maxdate and tiOversample_flag=1  
 group by survey_id) o  
  on (surv.survey_id=o.survey_id)  
 left outer join   
 (select survey_id,  
 ...
 left outer join  
 (select survey_id,  
 dbo.FormatDatetoString(datdaterange_fromdate)+' - '+dbo.FormatDatetoString(datdaterange_todate) [LastSampEncDateRange]  
 from sampleset   
 where sampleset_id in (select max(sampleset_id) from sampleset where datsamplecreate_dt between @mindate and @maxdate   
       and tiOverSample_Flag=0 group by survey_id)) dr  
 on surv.survey_id=dr.survey_id  
  
--find sent information  
insert #mailed (survey_id,samplesmailed,firstsmailed)  
select ms.survey_id,  
 count(distinct ms.sampleset_id),  
 sum(ms.mailcount)  
...
set @sql=''  
set @sql=  
'select  e.stremployee_first_nm+'' ''+e.stremployee_last_nm [PM],'+char(10)+  
 ' css.strclient_nm [Client],css.client_id,css.strstudy_nm [Study],css.study_id,'+char(10)+  
 ' case when sd.surveytype_id=2 then css.strSurvey_nm+'' (H)'' '+char(10)+ 
 ' when sd.surveytype_id=3 then css.strSurvey_nm+'' (HH)'' '+char(10)+ 
 ' when sd.surveytype_id=4 then css.strSurvey_nm+'' (MN)'' '+char(10)+ 
 ' else css.strSurvey_nm end [Survey],'+char(10)+  
 ' css.survey_id,'+char(10)+  
 ' case when sd.surveytype_id=2 then ''Yes'' else ''No'' end [HCAHPS],'+char(10)+  
 ' left(css.strSurvey_nm,4) [PJ#],'+char(10)+  
 ...
 ' isnull(psi.[Expected Samples],0) [Expected Samples],isnull(psi.[Completed Samples],0) [Completed Samples],'+char(10)+  
 ' isnull(psi.[Remaining Samples],0) [Remaining Samples],'+char(10)+  
 ' nsp.perioddef_id,'+char(10)+  
 ' isnull(nsp.datNextSample,''4000-12-31'') [Next Sample Date],'+char(10)+  
 ' isnull(nsp.[Period Name],''Unknown'') [Period Name],'+char(10)+  
 ' isnull(nsp.[Sampling Freq],''Unknown'') [Sampling Freq],'+char(10)+  
 ' nsp.PeriodExpSamps [Period Expected Samples],'+char(10)+  
 ' isnull(s.[Total Samples],0) [Total Samples],isnull(s.[Samples Rolled Back],0) [Samples Rolled Back],'+char(10)+  
 ' isnull(s.[Oversamples],0) [Oversamples],'+char(10)+  
 ' isnull(s.[Samples Scheduled],0) [Samples Scheduled],'+char(10)+  
 ' isnull(s.[LastSampEncDateRange],''Not Specified'') [LastSampEncDateRange],'+char(10)+  
...
'from (select distinct strclient_nm,client_id,strstudy_nm,study_id,strsurvey_nm,survey_id from clientstudysurvey_view) css'+char(10)+  
 'inner join #surveys surv'+char(10)+  
 ' on (css.survey_id=surv.survey_id)'+char(10)+  
 'left outer join #PeriodSampleInfo psi'+char(10)+  
 ' on (css.survey_id=psi.survey_id)'+char(10)+  
 'left outer join #NextSamplesPeriod nsp'+char(10)+  
 ...
 ' on (css.survey_id=sd.survey_id)'+char(10)+  
 +'where '+@criteria+char(10)+  
'order by [Client],left(css.strSurvey_nm,4),[Study],[Survey]'  
print @sql  
exec (@sql)  
  
/***/--select '' surveys,* from #surveys order by survey_id  
