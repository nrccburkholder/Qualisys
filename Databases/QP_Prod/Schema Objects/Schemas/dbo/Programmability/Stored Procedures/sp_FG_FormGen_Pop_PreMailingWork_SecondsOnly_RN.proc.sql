CREATE PROCEDURE sp_FG_FormGen_Pop_PreMailingWork_SecondsOnly_RN
AS
...
FROM   Survey_def SD(NOLOCK), ScheduledMailing SM(NOLOCK), MailingStep ms(nolock), SamplePop SP(NOLOCK) 
WHERE  SM.SentMail_id IS NULL AND
       ...
       MS.MailingStep_id = SM.MailingStep_id AND
