 ...
 *
 ******************************************************************************/
CREATE PROCEDURE dbo.PU_MN_ExtractPeriod1
AS
  SET ANSI_NULLS ON
  ...
  SELECT DISTINCT
    ...
    FROM ( -- Pull all the period
          SELECT pe.Survey_ID,
            ...
            FROM Period pb,      -- period begin date point
                 Period pe       -- period end date point
  ...
  -- Delete the old data
  TRUNCATE TABLE Last2Period1

  IF (@@ERROR <> 0) RETURN 2
  

  -- Current period
  INSERT INTO Last2Period1 (
          Survey_ID,
          PeriodType,
          PeriodBeginDate,
          PeriodEndDate
