/****** Object:  Stored Procedure dbo.SP_LiveDataStats    Script Date: 6/9/99 4:36:36 PM ******/
CREATE PROCEDURE SP_LiveDataStats @study_id NUMERIC
AS
BEGIN
   SET NOCOUNT ON
   ...
   BEGIN
      SELECT @workingString = 
                'SELECT ' + @field_nm + ' = COUNT(*) ' +
                  ' FROM S' + CONVERT(VARCHAR, @study_id) + '.' + @table_nm  +
