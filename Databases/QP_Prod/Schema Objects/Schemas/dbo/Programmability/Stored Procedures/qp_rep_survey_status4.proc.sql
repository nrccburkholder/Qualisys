CREATE procedure qp_rep_survey_status4
...
select strmailingstep_nm as 'Mailing Step', count(*) as 'OutGo', sum(case when qf.datreturned is null then 0 else 1 end) as 'Returns', 
	sum(case when sm.datundeliverable is null then 0 else 1 end) as 'Nondels',
	100.0*sum(case when qf.datreturned is null then 0 else 1 end) / (count(*) - sum(case when sm.datundeliverable is null then 0 else 1 end)) as 'Reponse Rate'
from sentmailing sm, scheduledmailing schm, samplepop sp, sampleset ss, mailingstep ms, questionform qf
...
group by strmailingstep_nm, intsequence
