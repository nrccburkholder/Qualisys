CREATE Procedure [dbo].[SP_SplitExportDBF_DM]
...
set @MRDTable_nm = 'MRD_' + convert(varchar,@Cutoff_id)

select @MRDTable_id = id from datamart.qp_comments.dbo.sysobjects where name = @MRDTable_nm
...
into #Background
from datamart.qp_comments.dbo.syscolumns sc, datamart.qp_comments.dbo.systypes st
...
begin
 ...
 begin
  ...
  begin
   set @SQL = @SQL + ')'
   set @SQLInsert = @SQLInsert + ' from datamart.qp_comments.s' + @strStudy_id + '.' + @MRDTable_nm
  end
  else
  begin
   set @SQL = @SQL + ', '
...
set @SQL = 'drop table datamart.qp_comments.s' + @strStudy_id + '.' + @MRDTable_nm
