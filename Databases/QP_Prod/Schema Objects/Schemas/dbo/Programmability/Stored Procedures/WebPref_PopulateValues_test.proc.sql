/****** Object:  Stored Procedure dbo.WebPref_PopulateValues    Script Date: 12/23/2004 11:09:37 AM ******/
CREATE PROCEDURE WebPref_PopulateValues_test
AS
...
BEGIN
	...
	SELECT @sql=@sql+ViewField+' VARCHAR(42),' FROM #Fields
...
--		SELECT @sql='INSERT INTO WebSurveyValues (Survey_id, BarCode, MethodName, MethodValue)
	select @sql='		SELECT Survey_id, BarCode, '''+@MethodName+''','+@ViewName+'
			FROM #Values'
-- PRINT @sql	
 		EXEC (@sql)
