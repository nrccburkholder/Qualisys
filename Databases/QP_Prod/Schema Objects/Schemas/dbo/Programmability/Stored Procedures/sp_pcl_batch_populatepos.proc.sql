...
AS
 ...
 while exists (select batch_id
			   ...
			   and Questionform_id not in (	select Questionform_id
											from qp_prod.dbo.PCLGenPosError
											where isResolved = 0 )
				) 
 begin
/*
...
*/
  ...
  and questionform_id not in (	select questionform_id
								 from qp_prod.dbo.PCLGenPosError
								 where isResolved = 0)	 
  exec dbo.sp_pcl_populate_pos_tables @batch_id
  select @err = @@error
  if @err <> 0
  begin
	INSERT INTO qp_scan.dbo.PCLGenPosError (batch_id, sql_error, returned_error)
 ...
 if exists (select batch_id
			from qp_scan.dbo.PCLGenPosError
