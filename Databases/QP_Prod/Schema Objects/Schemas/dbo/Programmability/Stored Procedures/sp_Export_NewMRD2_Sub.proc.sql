CREATE procedure sp_Export_NewMRD2_Sub
@cutoff_id integer, @ReturnsOnly bit = 0 
as

exec master.dbo.xp_sendmail @recipients='bdohmen', @subject='sp_Export_NewMRD2_Sub', @message=@Cutoff_id
...
select @SQL = 'ALTER TABLE s' + convert(varchar,@study_id)+'.MRD_'+convert(varchar,@cutoff_id)+' ADD '
open curDef
...
select @SQL = 'INSERT INTO s' + convert(varchar,@study_id)+'.MRD_'+convert(varchar,@cutoff_id)+' (sampset, samp_dt, sampunit, unit_nm, samptype, samppop'
...
'from s' + convert(varchar,@study_id)+'.MRD_'+convert(varchar,@cutoff_id)+' MRD, QuestionForm qf, sentmailing sm '+
...
if @cutofftype = '1'
  ...
  'from s' + convert(varchar,@study_id)+'.MRD_'+convert(varchar,@cutoff_id)+' mrd, QuestionForm qf '+
  'where mrd.samppop=qf.samplepop_id '+
...
begin
  select @SQL = 
      ...
      'from s' + convert(varchar,@study_id)+'.MRD_'+convert(varchar,@cutoff_id)+' MRD, questionresult qr '+
