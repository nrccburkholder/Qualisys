...
*/
CREATE procedure sp_Export_NewMRD3_DoDBen_Sub_Second
@cutoff_id integer, @ReturnsOnly bit = 0, @DirectOnly bit = 1
as

--exec master.dbo.xp_sendmail @recipients='bdohmen', @subject='sp_Export_NewMRD3_DoDBen_Sub_First', @message=@Cutoff_id
...
begin
  if @cutofftype='0'
    ...
    begin
      ...
      'from questionform qf, samplepop sp, selectedsample ss, s'+convert(varchar,@study_id)+'.big_view bv '+
      'where qf.cutoff_id='+convert(varchar,@cutoff_id)+
      '  and qf.samplepop_id=sp.samplepop_id'+
      '  and sp.sampleset_id=ss.sampleset_id'+
      '  and sp.pop_id=ss.pop_id'+
      '  and ss.enc_id=bv.encounterpop_id'
	  ELSE
      select @SQL = 
      'insert into #daterange (mn, mx) ' +
      'select min('+@sel+'), max('+@sel+') '+
...
select @Sel = 'select ss.sampleset_id, s.datSampleCreate_dt, ss.sampleunit_id, su.strsampleunit_nm, ss.strUnitSelectType, sp.samplepop_id, schm.Mailingstep_id'
/*
select @SQL = 'INSERT INTO s' + convert(varchar,@study_id)+'.MRD2_'+convert(varchar,@cutoff_id)+' (sampset, samp_dt, sampunit, unit_nm, samptype, samppop, mailstep'
...
select @SQL = @SQL + ') ' + @Sel + ' FROM SampleSet s, SelectedSample SS, SamplePop SP, SampleUnit SU '
--select @SQL = @SQL + ') ' + @Sel + ' FROM s'+convert(varchar,@study_id)+'.Unikeys UK, SampleSet s, SelectedSample SS, SamplePop SP, SampleUnit SU '
--if @ReturnsOnly=1
select @SQL = @SQL + ', QuestionForm QF, scheduledmailing schm '
...
IF EXISTS (SELECT * FROM MetaTable WHERE Study_id=@Study_id AND strTable_nm='ENCOUNTER')
select @SQL = @SQL + ' where ss.sampleset_id=s.sampleset_id and ss.sampleunit_id=su.sampleunit_id and ss.enc_id=Encounter.enc_id and ss.sampleset_id=sp.sampleset_id and ss.pop_id=sp.pop_id'
ELSE
select @SQL = @SQL + ' where ss.sampleset_id=s.sampleset_id and ss.pop_id=population.pop_id and ss.sampleunit_id=su.sampleunit_id and ss.sampleset_id=sp.sampleset_id and ss.pop_id=sp.pop_id' 
/*
...
/*Modified 4/24/1 Brian Dohmen  -- Changed questionresult2 to be #questionresult2*/
