CREATE PROCEDURE [ETL].[RecoverSamplePlanWorksheet]
	@ArchiveRunID INT,
	@NumToWork INT
AS
BEGIN
	...
	WHERE TaskCode = 'R_SPW' --Archive SamplePlanWorksheet
		AND [Type] = 'R'
...
INSERT INTO [QP_Prod].[dbo].[SamplePlanWorksheet]
  ...
  FROM [ARCHIVE].[SamplePlanWorksheet] s
	  INNER JOIN cteSP t ON t.[Sampleset_id] = s.[Sampleset_id]
	  LEFT JOIN [QP_Prod].[dbo].[SamplePlanWorksheet] a ON s.[SamplePlanWorksheet_id] = a.[SamplePlanWorksheet_id] 
