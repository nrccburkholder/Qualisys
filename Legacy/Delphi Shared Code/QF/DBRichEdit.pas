...
procedure TclDBRichCode.SortCodeList;
  { sorts Codelist by descending length of the replacement text, so the longest code will be analyzed first
    should prevent confusion between "you" and "you and your child" by doing the longer one first.}
var tmp : string;
    i,j : integer;
begin
  with CodeList do
    if count>1 then
      for i := count-2 downto 0 do
        for j := 0 to i do
          if (length(strings[j])-pos('=',strings[j])) < (length(strings[j+1])-pos('=',strings[j+1])) then begin
            tmp := strings[j+1];
