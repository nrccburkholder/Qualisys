...
{ TStretchHandles is a transparent control to implement runtime grab handles
  ...
  Modified by:		W L Kurtze, ShoeString Enterprises, March 1997.
...
type
  ...
  EBadChild = class(Exception);
													{ TWLKHandle component declaration }
type
  TWLKHandle = class(TCustomControl)
  private
//WLK additions
		FAspect: Single;
		FResizable: Boolean;
...
procedure Register;
begin
	{ add the component to the 'WLK' tab }
  RegisterComponents('WLK', [TWLKHandle]);

end;

constructor TWLKHandle.Create(AOwner: TComponent);
begin
...
procedure TWLKHandle.CreateParams(var Params: TCreateParams);
begin
...
procedure TWLKHandle.WMGetDLGCode(var Message: TMessage);
begin
...
procedure TWLKHandle.WMEraseBkgnd(var Message: TWMEraseBkgnd);
begin
...
procedure TWLKHandle.Attach(ChildControl: TControl);
var
  L, T, W, H: integer;
begin
  ...
  if ChildControl is TForm then
	 raise EBadChild.Create('Handles can not be attached to a Form!');
													{ add child component to unique list managed by TWLKHandle }
	 ...
	 begin
		...
		if FChildList.Count = 0 then
		  begin
			 ...
			 FAspect := ChildControl.Width / ChildControl.Height;
{			 if 	(ChildControl is TWLKImage) then
				begin
					 FResizable := TWLKImage(ChildControl).Resizable;
					 FProportional := TWLKImage(ChildControl).Proportional;
				end;
			 if 	(ChildControl is TWLKLabel) then
				begin
					 FResizable := TWLKLabel(ChildControl).Resizable;
					 FProportional := TWLKLabel(ChildControl).Proportional;
				end;
			 if 	(ChildControl is TWLKRETextBox) then
				begin
					 FResizable := TWLKRETextBox(ChildControl).Resizable;
					 FProportional := TWLKRETextBox(ChildControl).Proportional;
				end;}
...
procedure TWLKHandle.ReleaseChild(ChildControl: TControl);
var
  idx, L, T, W, H: integer;
  AControl: TControl;
begin
  ...
  if FChildList.Count = 0 then
	 ...
	 begin
		 AControl := TControl(FChildList.Items[0]);
		 FAspect := AControl.Width / AControl.Height;
{		 if 	(AControl is TWLKImage) then
			begin
				 FResizable := TWLKImage(AControl).Resizable;
				 FProportional := TWLKImage(AControl).Proportional;
			end;
		 if 	(AControl is TWLKLabel) then
			begin
				 FResizable := TWLKLabel(AControl).Resizable;
				 FProportional := TWLKLabel(AControl).Proportional;
			end;
		 if 	(AControl is TWLKRETextBox) then
			begin
				 FResizable := TWLKRETextBox(AControl).Resizable;
				 FProportional := TWLKRETextBox(AControl).Proportional;
			end;}
...
function TWLKHandle.IndexOf(ChildControl: TControl): integer;
begin
...
procedure TWLKHandle.BringToFront;
var
  i: integer;
begin
...
procedure TWLKHandle.SendToBack;
var
  i: integer;
begin
                                       { do nothing if not Attached }
...
procedure TWLKHandle.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
...
procedure TWLKHandle.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
  ARect: TRect;
begin
...
procedure TWLKHandle.MouseMove(Shift: TShiftState; X, Y: Integer);
var
  ARect: TRect;
...
begin
  ...
  if (ssLeft in Shift) and not FDragging and not FLocked then
    ...
    begin
		...
		if (FChildList.Count = 1) and FResizable then
			begin
...
procedure TWLKHandle.ForwardMessage(FwdMsg: TForwardMessage; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
  i: integer;
  Found: boolean;
  Msg: Word;
...
begin
  ...
  for i := 0 to Parent.ControlCount - 1 do
    begin
      ...
      if (AControl is TWinControl) and not (AControl is TWLKHandle) then
        begin
...
procedure TWLKHandle.KeyDown(var Key: Word; Shift: TShiftState);
begin
...
function TWLKHandle.GetModifiedRect(XPos, YPos: integer): TRect;
var
  ARect: TRect;
begin
...
procedure TWLKHandle.Rubberband(XPos, YPos: integer; ShowBox: boolean);
var
  NewRect: TRect;
  PtA, PtB: TPoint;
  ScreenDC: HDC;
...
procedure TWLKHandle.SetBounds(ALeft, ATop, AWidth, AHeight: Integer);
var
  WasVisible: boolean;
  i: integer;
  AControl: TControl;
...
procedure TWLKHandle.SetPrimaryColor(Color: TColor);
begin
...
procedure TWLKHandle.SetSecondaryColor(Color: TColor);
begin
...
procedure TWLKHandle.SetColors(Color1, Color2: TColor);
begin
...
procedure TWLKHandle.SetGridState(Value: boolean);
begin
...
function TWLKHandle.GetGridState: boolean;
begin
...
function TWLKHandle.GetChildCount: integer;
begin
  Result := FChildList.Count;
end;

function TWLKHandle.GetChildControl(idx: integer): TControl;
begin
...
function TWLKHandle.IsAttached: boolean;
begin
...
function TWLKHandle.PointOverChild(P: TPoint): boolean;
var
  i: integer;
  ARect: TRect;
  AControl: TControl;
...
function TWLKHandle.XGridAdjust(X: integer): integer;
begin
  Result := (X DIV FGridX) * FGridX;
end;

function TWLKHandle.YGridAdjust(Y: integer): integer;
begin
