    ...
    internal static class OcsFwParser
    {
        ...
        private static void ParseHeader(XDocument xml, IEnumerable<string> lines)
        {
            ...
            foreach (var line in lines)
            {
                if (IsHeader(line))
                {
                    if (foundHeader) throw new InvalidOperationException("More than one header line found in file.");
                    foundHeader = true;
            ...
            if (!foundHeader) throw new InvalidOperationException("No header line found in file.");
        }
