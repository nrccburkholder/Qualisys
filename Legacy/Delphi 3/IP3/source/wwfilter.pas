...
Function isArithOp(token: string): boolean;
begin
   result:=
	((token='+') or
	(token='-') or
...
TNode = Class
  ...
  public
    ...
    Procedure traverse;
    procedure addCompareNode(a_operation: CanOp);
...
procedure TNode.addCompareNode(a_operation: CanOp);
begin
   dec(curBinaryNodeCount);
...
{$ifdef win32}
   PTFilterNodeClass(FilterBufferPtr)^.filterNodeType := Integer(nodeCompare);
   PTFilterNodeClass(FilterBufferPtr)^.operation := Integer(a_operation);
{$else}
   PTFilterNodeClass(FilterBufferPtr)^.filterNodeType := Word(nodeCompare);
   PTFilterNodeClass(FilterBufferPtr)^.operation := Word(a_operation);
{$endif}
...
{ !!!!
   caseInsensitive:= 0;
   partialLength:= 0;
   PTFilterNodeClass(FilterBufferPtr)^.data1 := caseInsensitive
   PTFilterNodeClass(FilterBufferPtr)^.data2 := partialLength;
          SetNodeOp(Result, 2, PutFieldNode(Field));
          SetNodeOp(Result, 3, PutConstStr(S));
   PTFilterNodeClass(FilterBufferPtr)^.data3 :=
   PTFilterNodeClass(FilterBufferPtr)^.data4 := literalOffset;
}
   nodeOffset := curBinaryNodeCount*TFilterNodeClassSize;
end;
...
begin
   ...
   if (Table.TableType > ttASCII) then begin
      ...
      {$ifdef win32}
      hRocket := GetModuleHandle('SDE3032');
      if hRocket=0 then hRocket := GetModuleHandle('SDE2532');
      {$else}
      hRocket := GetModuleHandle('SDE3016');
