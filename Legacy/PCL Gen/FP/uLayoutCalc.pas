...
type
  ...
  private
    ...
    function PCLFont(const name:string; const size:integer; const style:TFontStyles):string;
...
begin
  ...
  begin
    ...
    result := PCLFont('Arial Narrow',size,style);
  end
  else
  if TurnOn then
      result := PCLItalicOn
        ...
        while (not eof) and (wwt_SclsID.value = Sid) and (nSkip<SkipMax) do begin
          ...
          if wwt_Skip.findkey([glbSurveyID,
            ...
            if (dmOpenQ.CurrentLanguage > 1) and (CAHPSNumbering or DoDBenSkips) then begin
              ...
              if pos(')',vScaleText)>0 then mydelete(vScaletext,pos(')',vScaleText),1);
            end;
            inc(nSkip);
          ...
          if (wwt_SclsMissing.asboolean) and (wwt_QstnsScalePos.AsInteger in [spRight,spBelow2]) then begin
            inc(Miss[0].val);
            Miss[Miss[0].val].text := vScaleText;
            ...
            inc(Resp[0].val);
            Resp[Resp[0].val].text := vScaleText;
...
function tfrmLayoutCalc.PCLFont(const name:string; const size:integer; const style:TFontStyles):string;
var fn : integer;
begin
  if name = 'Arial Narrow' then begin
    fn := 16537;
...
*)
  ...
  if mockup=ptCodeSheetMockup then begin
    ...
    result := result + PCLPopPush + PCLRelXY(RelX+50,RelY-10) +
          PCLFont('Arial Narrow',9,[]) + PCLCenter(inttostr(rDQShape[s].value)) +
...
function tfrmLayoutCalc.PCLTextBox(const PgType:integer):string;
  ...
  begin
    if (newName<>CF) or (newSize<>CS) or
      ...
      result := PCLFont(NewName,NewSize,attrib);
      CF := NewName;
      CS := NewSize;
...
var i,tbLeft,tbTop,tbWidth,tbHeight,tbBorder,tbShading,curX,curY : integer;
    ...
    ParaNum,CharNum,SplittedLines,offset:integer;
    PGraph : array[0..35] of tPGraph;
begin
  WriteRTF(dmOpenQ.wwT_TextBoxRichText,dmOpenQ.tempdir+'\richtext.rtf');
  personalizeRTF;
...
begin
  ...
  PCLStream := PCLFont(curFont,CurSize,[]);
  pnl := 1;
  shp := 1;
  lbl := 1;
  for i := 1 to nDQRichEdit do begin
    ...
    with rDQRichEdit[i] do begin
      ...
      if (CurFont<>Fontname) or (curSize<>fontsize) then begin
        r := r + PCLFont(Fontname,fontsize,curStyle);
        curFont := FontName;
        curSize := FontSize;
      end;
      ...
      then begin
        ...
        begin
          r := r + ESC + '*p+2x-2X' + PCLPopPush + PCLRelXY(5-left,-round(0.75*linespacing)) +
              PCLBox(clWhite,BorderWidth,-1,-1,((PageWidth-(ColumnGutter*(ColumnCnt-1))) div ColumnCnt) - 25,height);
        ...
        if scaleid > 0 then begin
          r := r + PCLPopPush + PCLRelXY(0,height-55-round(0.75*linespacing));
          CmntLineRelY := height-55-round(0.75*linespacing);
    ...
    if rDQPanel[pnl].question = i then begin
      with rDQPanel[pnl] do begin
        ...
        if rDQLabel[lbl].panel = pnl then begin
          ...
          SclStrm := PCLFont(Fontname,fontsize,curStyle);
          SclHt := 0;
          ...
          while pos('›',SclStrm)>0 do begin
            insert(PCLShapeFont+'>'+PCLFont(Fontname,fontsize,curStyle),SclStrm,pos('›',SclStrm));
...
begin
  ...
  with DMOpenQ, wwt_Qstns do begin
    ...
    if (findkey([-1,1,0])) then begin
      ...
      s := ESC + '*p<<X>>x<<Y>>Y' + PCLPush + PCLFont('Arial',11,[]);
        ...
        if richedit.lines[i] <> '' then begin
          ...
          begin
            ...
            if AllIn(richedit.lines[i],'|0123456789') then
              s := s + PCLRelXY(0,LineSpacing*L) + ESC + '(1X' + richedit.lines[i] + PCLFont('Arial',11,[]);
          end else
 ...
 // WritePrinter
  ...
  with DMOpenQ, wwt_Qstns do begin
    ...
    if findkey([-1,1,0]) then begin
      ...
      PostageStamp := '';
      L := 0;
      s := ESC + '*p<<X>>x<<Y>>Y' + PCLPush + PCLFont('Arial',10,[]);
      for i := 0 to richedit.lines.count-1 do begin
        if richedit.lines[i] <> '' then begin
...
begin
  case dmOpenQ.CurrentLanguage of
    2,7,9: QText := '# '; //Spanish, VA-Spanish, Harris County Spanish
    ...
    if bool then begin
      ...
      if TargetItem = 1 then begin
        if tPCL.findkey([TargetSect,TargetSub,0]) then
          ...
          case dmOpenQ.CurrentLanguage of
            2,7,8,9: TargetQnum := 'final'; //Spanish, VA Spanish, PEP-C Spanish, Harris County Spanish
            ...
            14: SkipError('Portuguese skip to end');
      ...
      while pos('$SVY',s)>0 do begin
        ...
        if pos(copy(s,b+1,2),'BF.BH.BC.BE')>0 then begin {address codes}
          if copy(s,b+1,2)='BF' then
            s2 := pclpush + pclrelxy(0,-100) + '$00' + pclpop + esc + '(1X' + '$||' + PCLFont('Arial',11,[])
  ...
  with dmOpenQ, wwt_scls do begin
    ...
    if findkey([Sid]) then begin
      ...
      while (not eof) and (result.id = wwt_sclsID.value) do begin
        inc(nlbl);
        WriteRTF(wwt_SclsRichText,tempdir+'\richtext.rtf');
        LoadRTF(tempdir+'\richtext.rtf');
...
begin
  with dmOpenq do begin
    ...
    wwt_Qstns.first;
    WriteRTF(wwt_QstnsRichText,tempdir+'\richtext.rtf');
