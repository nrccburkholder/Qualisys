...
-------------------------------------------------------------------------------
11-29-2006   GN01    Modified the file version display

*******************************************************************************}

interface

uses Windows, SysUtils, Classes, Consts;
    ...
    begin
      ...
      if GetFileVersionInfo(Pointer(FileName), 0, size, buffer) then
        ...
        begin
          ...
          //gn01: Result  := Format('Version %d.%d.%d%s%d%s', [Major, Minor, Release, '(Build', Build, ')']);
          Result  := Format('Version %d.%d%s%d%s', [Major, Minor,  '(Build', Build, ')']);
        end;
      FreeMem(buffer);
    end;
end;
