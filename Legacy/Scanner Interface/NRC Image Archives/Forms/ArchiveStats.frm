...
Begin VB.Form frmArchiveStats 
   Caption         =   "Archive Statistics"
   ClientHeight    =   4755
   ClientLeft      =   555
   ClientTop       =   510
   ClientWidth     =   7530
   Icon            =   "ArchiveStats.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   4755
   ...
   Begin GSEnhListView.GSListView lvwStats 
      ...
      LabelEdit       =   1
      MouseIcon       =   "ArchiveStats.frx":0442
      View            =   3
   ...
   End
End
Attribute VB_Name = "frmArchiveStats"
...
Private Sub LoadListView()
    ...
    sSql = "SELECT sArchiveLabel, Count(DISTINCT sTemplateLabel) AS lQtyTemplates, " & _
           "       Count(DISTINCT sTemplateLabel+sBatchLabel) AS lQtyBatches, " & _
           "       Count(*) AS lQtyImages, MIN(dtArchiveDate) AS dtStartTime, " & _
           "       MAX(dtArchiveDate) As dtFinishTime " & _
           "FROM ImageArchive " & _
           "GROUP BY sArchiveLabel " & _
           "ORDER BY sArchiveLabel"
    ...
    With oTempRS
        ...
        Do Until .EOF
            ...
            lArchCnt = lArchCnt + 1
            lTemplateCnt = lTemplateCnt + !lQtyTemplates
            lBatchCnt = lBatchCnt + !lQtyBatches
            lImageCnt = lImageCnt + !lQtyImages
            
            'Update the status bar
            sbrMain.Panels("Status").Text = "Retrieving record " & lArchCnt & "..."
            sbrMain.Refresh
            
            'Add this entry
            Set oItem = lvwStats.ListItems.Add(, "AL" & !sArchiveLabel, "" & !sArchiveLabel, , "Image")
            oItem.SubItems(1) = !lQtyTemplates
            oItem.SubItems(2) = !lQtyBatches
            oItem.SubItems(3) = !lQtyImages
            oItem.SubItems(4) = Format(!dtStartTime, "mm/dd/yyyy hh:nn:ss")
            oItem.SubItems(5) = Format(!dtFinishTime, "mm/dd/yyyy hh:nn:ss")
    ...
    sbrMain.Panels("Status").Text = lArchCnt & " Archive(s), " & lTemplateCnt & " Template(s), " & lBatchCnt & " Batch(es), " & lImageCnt & " Image(s)"
...
Private Sub Form_Load()
    ...
    With lvwStats
        ...
        With .ColumnHeaders
            ...
            .Add , "QtyTemplates", "Templates", 1080, gslvColumnRight, gslvAsNumber
...
Private Sub lvwStats_DblClick()
    ...
    ShowTemplateProperties sArchiveLabel:=oItem.Text, _
                           oParentForm:=Me
