...
Begin VB.Form frmBatchStats 
   Caption         =   "Batch Statistics"
   ClientHeight    =   4755
   ClientLeft      =   555
   ClientTop       =   510
   ClientWidth     =   7530
   Icon            =   "BatchStats.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   4755
   ...
   End
End
Attribute VB_Name = "frmBatchStats"
...
Public Property Let TemplateLabel(ByVal sData As String)
    ...
    Me.Caption = "Batch Statistics for Archive '" & msArchiveLabel & "', Template '" & msTemplateLabel & "'"
...
Private Sub LoadListView()
    ...
    sSql = "SELECT sBatchLabel, Count(*) AS lQtyImages, " & _
           "       MIN(dtArchiveDate) AS dtStartTime, " & _
           "       MAX(dtArchiveDate) As dtFinishTime " & _
           "FROM ImageArchive " & _
           "WHERE sArchiveLabel = '" & msArchiveLabel & "' " & _
           "  AND sTemplateLabel = '" & msTemplateLabel & "' " & _
           "GROUP BY sBatchLabel " & _
           "ORDER BY sBatchLabel"
    ...
    With oTempRS
        ...
        Do Until .EOF
            ...
            sbrMain.Panels("Status").Text = "Retrieving record " & lBatchCnt & "..."
            sbrMain.Refresh
            
            'Add this entry
            Set oItem = lvwStats.ListItems.Add(, "BL" & !sBatchLabel, "" & !sBatchLabel, , "Image")
            oItem.SubItems(1) = !lQtyImages
            oItem.SubItems(2) = Format(!dtStartTime, "mm/dd/yyyy hh:nn:ss")
            oItem.SubItems(3) = Format(!dtFinishTime, "mm/dd/yyyy hh:nn:ss")
    ...
    sbrMain.Panels("Status").Text = lBatchCnt & " Batch(es), " & lImageCnt & " Image(s)"
...
Private Sub Form_Load()
    ...
    With lvwStats
        ...
        With .ColumnHeaders
            .Add , "BatchLabel", "Batch", 1560, gslvColumnLeft, gslvAsText
...
Private Sub lvwStats_DblClick()
    ...
    ShowImageProperties sArchiveLabel:=msArchiveLabel, _
                        sTemplateLabel:=msTemplateLabel, _
                        sBatchLabel:=oItem.Text, _
                        oParentForm:=Me
