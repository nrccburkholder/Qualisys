        ...
        public static FileFormat Classify(string fileName, QP_DataLoadManager dataLoadManager)
        {
            ...
            switch (source)
            {
                ...
                case FileSource.DirectFromClient:
                    ...
                    if (clientFormat == null) throw new InvalidOperationException(string.Format("Can't get the file format for file {0} because there is no mapping of CCN to file format in the ClientFormat table.", fileName));

                    return (FileFormat)Enum.Parse(typeof(FileFormat), clientFormat.Format);
                default:
                    throw new InvalidOperationException(string.Format("Can't get the file format for file {0} because the file name is not in a recognized format.", fileName));
            }
        }
    }
}
