...
Public Class TranslatorTABCCAC
    Inherits Translator
...
#Region "Public Methods"
    ...
    Public Overrides Function Translate(ByVal queueFile As QueuedTransferFile) As DataLoad
        ...
        Dim table As DataTable = TranslatorProvider.Instance.GetDataTable(queueFile, TranslatorTypes.TABCCAC)
...
#Region "Private Methods"
    ...
    Protected Overrides Function PopulateDataSet(ByVal queueFile As QueuedTransferFile, ByVal table As DataTable) As DataSet
        ...
        loadSet.Tables("FileInfo").Rows.Add("1.0", "ADV")
        ...
        For Each row As DataRow In table.Rows
            ...
            If Not IsDataRowEmpty(row) Then
                ...
                Dim surveyIDError As String = String.Empty
                Dim langID As Nullable(Of Integer)
                Dim surveyId As Integer

                'Get the survey_id
                surveyId = TranslatorProvider.Instance.GetAdvanisSurveyID(row(GetFixedColumnName("ccac_code", queueFile)).ToString.Trim, _
                                                                                          row(GetFixedColumnName("unique_client_number", queueFile)).ToString.Trim(), _
                                                                                          queueFile.Translator.StudyId)

                'Validate the survey ID
                If surveyId = 0 Then
                    surveyIDError = String.Format("Survey_ID cannot be found for CCAC_CODE {0} and UNIQUE_CLIENT_NUMBER {1}", row(GetFixedColumnName("ccac_code", queueFile)).ToString.Trim, row(GetFixedColumnName("unique_client_number", queueFile)).ToString)
                    errorList.Add(New TranslatorError(rowCount, "N/A", surveyIDError))
                    ' if we can't find a survey_id, no need to go on with this record.
                    Continue For
                End If

                'Determine the LithoCode
                'Try to get the LithoCode using the ccac_code and unique_client_id
                lithoCode = TranslatorProvider.Instance.GetAdvanisLithoCode(row(GetFixedColumnName("ccac_code", queueFile)).ToString.Trim, _
                                                                                          row(GetFixedColumnName("unique_client_number", queueFile)).ToString.Trim(), _
                                                                                          queueFile.Translator.StudyId, surveyId)

                'Validate the LithoCode
                If String.IsNullOrEmpty(lithoCode) Then

                    lithoCodeError = String.Format("LithoCode cannot be found for CCAC_CODE {0} and UNIQUE_CLIENT_NUMBER {1}", row(GetFixedColumnName("ccac_code", queueFile)).ToString.Trim, row(GetFixedColumnName("unique_client_number", queueFile)).ToString)

                    lithoCode = "BLANK" & rowCount.ToString
                    errorList.Add(New TranslatorError(rowCount, lithoCode, lithoCodeError))
                    ' If we can't find a litho, no need to go on with this record.
                    Continue For
                End If
                ...
                If loadSet.Tables("Respondent").Select(String.Format("LithoCode = '{0}'", lithoCode)).GetLength(0) = 0 Then
                    ...
                    loadSet.Tables("Respondent").Rows.Add(surveyId, lithoCode, "PHONE", langID)
                Else
                    'This LithoCode already exists in the table so add it with an error

                    lithoCodeDup = String.Format("LithoCode {0} for CCAC_CODE ({1}) {2} and UNIQUE_CLIENT_NUMBER ({3}) {4} already exists in the file.", lithoCode, GetFixedColumnName("ccac_code", queueFile), row(GetFixedColumnName("ccac_code", queueFile)).ToString.Trim, GetFixedColumnName("unique_client_number", queueFile), row(GetFixedColumnName("unique_client_number", queueFile)).ToString)
