...
#Region " Private Methods "
        ...
        Private Sub DeleteImage()
            ...
            mImagePath = ""
            If Not mScreenShot Is Nothing Then
                mScreenShot.Dispose()
                mScreenShot = Nothing
            End If
        End Sub
        ...
        Private Function IsFileLocked(ByVal filePath As String) As Boolean
            ...
            Try
                Using stream As FileStream = File.Open(filePath, FileMode.Append, FileAccess.Write)
                    stream.Close()
                    Return False
                End Using
        ...
        Private Sub EmailReport()
            ...
            Using mail As New MailMessage
                Using attach As New Attachment(mImagePath)
                    mail.Attachments.Add(attach)
                    For Each addr As String In mReportRecipient.Split(New String() {",", ";"}, StringSplitOptions.RemoveEmptyEntries)
                        mail.To.Add(addr)
                    Next
                    mail.From = New MailAddress(ReportSender)
                    mail.Subject = mReportSubject
                    mail.Body = GetExceptionHtml()
                    mail.IsBodyHtml = True

                    Dim smtp As New System.Net.Mail.SmtpClient(mSMTPServer)
                    smtp.Send(mail)
                End Using
            End Using
        ...
        Private Function GetExceptionHtml() As String
            ...
            sb.AppendFormat("<TR><TD Class=""LabelCell"">Exception:</TD><TD Class=""DataCell"">{0}</TD></TR>", mException.Message.Replace(vbCrLf, "<BR>"))
            If TypeOf mException Is Nrc.Framework.Data.SqlCommandException Then
                sb.AppendFormat("<TR><TD Class=""LabelCell"">SQL Command:</TD><TD Class=""DataCell"">{0}</TD></TR>", DirectCast(mException, Nrc.Framework.Data.SqlCommandException).CommandText)
            End If
            ...
            If mException.StackTrace IsNot Nothing Then
                stack = mException.StackTrace.Replace("   at", "<BR>&nbsp;&nbsp;at")
            ...
            Dim InnerException As Exception = mException.InnerException
            Do While InnerException IsNot Nothing
                If (inStack <> "") Then inStack &= "<BR>"

                If (InnerException.Message IsNot Nothing) Then
                    inStack &= "--------Inner Exception--------" & "<BR>" & _
                               InnerException.Message.Replace(vbCrLf, "<BR>") & "<BR>"
                End If

                If (InnerException.StackTrace IsNot Nothing) Then
                    inStack &= InnerException.StackTrace.Replace("   at", "<BR>&nbsp;&nbsp;at") & "<BR>"
                End If

                InnerException = InnerException.InnerException
            Loop
            sb.AppendFormat("<TR><TD Class=""LabelCell"">Inner Exception:</TD><TD Class=""DataCell"">{0}</TD></TR>", inStack)
...
#Region " Windows Form Designer generated code "
            ...
            <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
                Dim resources As System.Resources.ResourceManager = New System.Resources.ResourceManager("Nrc.Framework.WinForms.ExceptionReport", System.Reflection.Assembly.GetCallingAssembly)
...
#End Region
            ...
            Private Sub LoadExceptionMessage()
                ...
                mIsSqlCommandException = (TypeOf mException Is Nrc.Framework.Data.SqlCommandException)
                Me.Caption = mTitle
                ...
                Dim inException As String = ""
                If mException.StackTrace IsNot Nothing Then
                    inException = mException.StackTrace & vbCrLf
                End If

                Dim innerException As Exception = mException.InnerException
                Do While innerException IsNot Nothing
                    If (inException <> "") Then inException &= vbCrLf
                    If innerException.Message IsNot Nothing Then
                        inException &= "--------Inner Exception--------" & vbCrLf & _
                                       innerException.Message & vbCrLf & vbCrLf
                    End If

                    If innerException.StackTrace IsNot Nothing Then
                        inException &= innerException.StackTrace & vbCrLf
                    End If

                    innerException = innerException.InnerException
                Loop

                txtStackTrace.Text = inException

                If mIsSqlCommandException Then
                    txtSqlCommand.Text = DirectCast(mException, Nrc.Framework.Data.SqlCommandException).CommandText
                End If
...
#Region " Windows Form Designer generated code "
            ...
            <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
                ...
                Me.txtRecipient.Text = "nrcsn@service-now.com"
                '
                'Label2
                '
