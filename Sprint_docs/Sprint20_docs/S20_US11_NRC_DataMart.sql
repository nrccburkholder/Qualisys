...
S20 US9  As an authorized Hospice CAHPS vendor, we must be able to report the sampling type (census or simple random)
...
if not exists (	SELECT 1 
				...
				WHERE  st.schema_id = 1 
					   AND st.NAME = 'SampleSet' 
					   AND sc.NAME = 'SamplingMethodID' )

	alter table [dbo].[SampleSet] add SamplingMethodID int NULL 
...
if not exists (	SELECT 1 
	...
	alter table [LOAD_TABLES].[SampleSet] add SamplingMethodID int NULL
...
if not exists (	SELECT 1 
	...
	alter table [LOAD_TABLES].[SampleSetError] add SamplingMethodID int NULL
