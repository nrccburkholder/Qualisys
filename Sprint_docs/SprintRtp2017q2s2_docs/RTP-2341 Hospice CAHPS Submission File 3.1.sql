...
go
if not exists (	select * from cem.ExportTemplate 
...
begin
	...
	set ExportTemplateVersionMajor='3.1'
  ...
  <xs:element name="vendordata">
        ...
        <xs:element name="hospicedata" maxOccurs="unbounded" minOccurs="0">
          <xs:complexType>
            <xs:sequence>
              ...
              <xs:element type="xs:string" name="available-sample" />
              <xs:element type="xs:string" name="sampled-cases" />
	...
	if not exists (select * from cem.ExportTemplateColumn where ExportTemplateSectionID=@hospicesectionid and ExportColumnName='available-sample')
		...
		values (@hospicesectionid, '<total-decedents> - ( <no-publicity> + <missing-dod> + <ineligible-presample> )', 18, 0, 'available-sample', '', 56, NULL, NULL, 10, null, null, .95, 0)
	
	--*Added “sampled-cases” XML Element
	-- actually, just renaming sample-size to sampled-cases 
	update cem.ExportTemplateColumn 
	set ExportColumnName='sampled-cases' 
	where ExportColumnName='sample-size'
	and ExportTemplateSectionID=@hospicesectionid 
	and DatasourceID=3

	--*Revised definition of “sample-size” XML Element 
	-- add a new sample-size
	if not exists (select * from cem.ExportTemplateColumn where ExportTemplateSectionID=@hospicesectionid and ExportColumnName='sample-size' and DatasourceID=0)
		insert into cem.ExportTemplateColumn (ExportTemplateSectionID, ExportTemplateColumnDescription, ColumnOrder, DatasourceID, ExportColumnName, SourceColumnName, SourceColumnType, AggregateFunction, DispositionProcessID, FixedWidthLength, ColumnSetKey, FormatID, MissingThresholdPercentage, CheckFrequencies)
		values (@hospicesectionid, '<sampled-cases> - ( <ineligible-presample + <ineligible-postsample>)', 19, 0, 'sample-size', '', 56, NULL, NULL, 10, null, null, .95, 0)

	update cem.ExportTemplateColumn
	set ColumnOrder=case ExportColumnName
		when 'missing-dod' then 12
		when 'ineligible-presample' then 13
		when 'available-sample' then 14
		when 'sampled-cases' then 15
		when 'sample-size' then 16
		when 'ineligible-postsample' then 17
		when 'sample-type' then 18
		when 'number-offices' then 19
		else ColumnOrder
	end
	...
	if not exists (select * from cem.ExportTemplateColumnResponse where Exporttemplatecolumnid=@langCol and RawValue=6)
		insert into cem.ExportTemplateColumnResponse (Exporttemplatecolumnid, RawValue, RecodeValue, ResponseLabel)
		values (@langCol, 6, ' 6', 'Vietnamese')

	if not exists (select * from cem.ExportTemplateColumnResponse where Exporttemplatecolumnid=@langCol and RawValue=7)
		insert into cem.ExportTemplateColumnResponse (Exporttemplatecolumnid, RawValue, RecodeValue, ResponseLabel)
		values (@langCol, 7, ' 7', 'Polish')

	if not exists (select * from cem.ExportTemplateColumnResponse where Exporttemplatecolumnid=@langCol and RawValue=8)
		insert into cem.ExportTemplateColumnResponse (Exporttemplatecolumnid, RawValue, RecodeValue, ResponseLabel)
		values (@langCol, 8, ' 8', 'Korean')
...
update eds
SET [hospicedata.sample-size] = convert(int,[hospicedata.sampled-cases]) - (convert(int,[hospicedata.ineligible-presample]) + convert(int,[hospicedata.ineligible-postsample]))
FROM [NRC_Datamart_Extracts].[CEM].[ExportDataset00000019] eds
where eds.ExportQueueID = @ExportQueueID
and eds.[hospicedata.sample-size]=''

update eds
SET [hospicedata.available-sample] = convert(int,[hospicedata.total-decedents]) - (convert(int,replace([hospicedata.no-publicity],'M','0')) + convert(int,[hospicedata.missing-dod]) + convert(int,[hospicedata.ineligible-presample]))
FROM [NRC_Datamart_Extracts].[CEM].[ExportDataset00000019] eds
where eds.ExportQueueID = @ExportQueueID
and eds.[hospicedata.available-sample]=''
go