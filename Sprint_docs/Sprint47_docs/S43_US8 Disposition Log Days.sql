/*
	...
	S43 US 8 CAHPS: Disposition Log Days 
	As the Atlas Team, we want to populate days from first in the NRC10 dispo log, so that dispositions are processed.



	ALTER PROCEDURE [dbo].[CheckForCAHPSIncompletes]

	Changed table structure on temp_qfResponseCount and temp_TodaysReturns to include columns DaysFromFirst and DaysFromCurrent

	Tim Butler
	
	This was not deployed with the rest of Sprint 43. 
	I added core 54067 to the hospice cahps completion formula (which was done last week as part of ticket support)
...
if not exists (	SELECT 1 
				...
				WHERE  st.schema_id = 1 
					   AND st.NAME = 'temp_TodaysReturns' 
					   AND sc.NAME = 'DaysFromFirst' )
BEGIN
	alter table [dbo].[temp_TodaysReturns] add DaysFromFirst int NULL 
END
...
if not exists (	SELECT 1 
				...
				WHERE  st.schema_id = 1 
					   AND st.NAME = 'temp_TodaysReturns' 
					   AND sc.NAME = 'DaysFromCurrent' )
BEGIN
	alter table [dbo].[temp_TodaysReturns] add DaysFromCurrent int NULL 
END
...
if not exists (	SELECT 1 
				FROM   sys.tables st 
					   INNER JOIN sys.columns sc ON st.object_id = sc.object_id 
				WHERE  st.schema_id = 1 
					   AND st.NAME = 'temp_qfResponseCount' 
					   AND sc.NAME = 'DaysFromFirst' )
BEGIN
	alter table [dbo].[temp_qfResponseCount] add DaysFromFirst int NULL 
END
...
if not exists (	SELECT 1 
				...
				WHERE  st.schema_id = 1 
					   ...
					   AND sc.NAME = 'DaysFromCurrent' )
BEGIN
	alter table [dbo].[temp_qfResponseCount] add DaysFromCurrent int NULL 
END


USE [QP_Prod]
GO
/****** Object:  StoredProcedure [dbo].[CheckForCAHPSIncompletes]    Script Date: 2/18/2016 8:56:12 AM ******/
SET ANSI_NULLS ON
GO
...
-- ACO Disposition 34 = blank/incomplete
	...
	*/
	...
	inner join (select qr.QuestionForm_id, count(distinct sq.qstncore) as ATACnt
				...
				inner join sel_qstns sq on qr.survey_id = sq.survey_id and qr.qstncore = sq.qstncore 
				inner join sel_scls ss on sq.scaleid = ss.qpc_id AND sq.survey_id = ss.survey_id and qr.intresponseval = ss.val
				where sq.qstncore in (51574,51575,51576,51577,51579,51580,51581,51582,51583,51584,51585,51586,51588,51590,51594,51597,51599,51601,51603,51604,51605,51608,51609,51610,51611,51612,51613,51614,51615,51616,51617,51618,51619,51620,54067)
				and sq.subtype = 1 
				AND sq.language = 1 
				AND ss.language = 1 
				group by qr.QuestionForm_id) rc
