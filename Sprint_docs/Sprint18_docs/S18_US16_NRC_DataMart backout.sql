...
if exists (	SELECT 1 
	...
	alter table [LOAD_TABLES].[SampleSet] drop column IneligibleCount
...
if exists (	SELECT 1 
	...
	alter table [LOAD_TABLES].[SampleSetError] drop column IneligibleCount
...
/****** Object:  StoredProcedure [dbo].[etl_LoadSampleSetRecords]    Script Date: 2/6/2015 1:23:26 PM ******/
SET ANSI_NULLS ON
GO
...
--			2.0  Tim Butler - S14.2 US11 Add StandardMethodologyID column to SampleSet
-- =============================================
BEGIN	
    ...
    */
	...
	INSERT dbo.SampleSet
		(SampleSetID, ClientID, SampleDate,StandardMethodologyID)
		SELECT SampleSetID, ClientID, CONVERT(DATE,SampleDate),StandardMethodologyID -- 2.0
		  FROM LOAD_TABLES.SampleSet WITH (NOLOCK)
		 WHERE DataFileID = @DataFileID
		   AND isInsert <> 0 AND isDelete = 0 AND ClientID IS NOT NULL
