use qp_prod
go
if object_id('[dbo].[SP_FG_OutreachVendor]') is not null
	DROP PROCEDURE [dbo].[SP_FG_OutreachVendor]
go
if exists (select * from sys.tables st join sys.columns sc on sc.object_id=st.object_id where st.schema_id=1 and st.name='languages' and sc.name='ISO639')
	alter table DBO.Languages drop column ISO639
go
