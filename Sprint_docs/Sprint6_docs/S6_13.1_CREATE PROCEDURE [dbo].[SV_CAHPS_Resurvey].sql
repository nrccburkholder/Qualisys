...
/****** Object:  StoredProcedure [dbo].[SV_CAHPS_Resurvey]    Script Date: 8/13/2014 1:43:05 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE TABLE #M (Error TINYINT, strMessage VARCHAR(200))
	...
	BEGIN
		--check resurvey Exclusion Type
		INSERT INTO #M (Error, strMessage)
		SELECT 1,'Resurvey type is not set to Days.'
		FROM Survey_def
		WHERE Survey_id=@Survey_id
		AND ReSurveyMethod_id<>1
		...
		--Check resurvey Exclusion Period
		INSERT INTO #M (Error, strMessage)
		SELECT 1,'Resurvey Days is not 365.'
		FROM Survey_def
		WHERE Survey_id=@Survey_id
		AND INTRESURVEY_PERIOD<>365
	END
