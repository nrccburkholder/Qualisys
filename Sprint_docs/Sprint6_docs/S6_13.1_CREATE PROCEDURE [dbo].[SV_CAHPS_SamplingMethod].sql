...
/****** Object:  StoredProcedure [dbo].[SV_CAHPS_SamplingMethod]    Script Date: 8/13/2014 1:45:57 PM ******/
SET ANSI_NULLS ON
GO
...
IF @surveyType_id in (@CGCAHPS) AND @subtype_id = @PCMHSubType
	BEGIN
		--What is the sampling method                                                        
		if exists(select 1 from PeriodDef where survey_id=@survey_id and SamplingMethod_id <> (select SamplingMethod_id from SamplingMethod where strSamplingMethod_nm = 'Specify Outgo'))
		INSERT INTO #M (Error, strMessage)
		SELECT 1,'All sample periods must be Specify Outgo sampling.'
	END
