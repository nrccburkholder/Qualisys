...
-- 12/17/2014 by dgilsdorf: added -4 (appropriately skipped) as an equivalent to -9 (not answered)
-- =============================================
ALTER PROCEDURE [dbo].[GetExportData] 
	@SurveyId int,
	@StudyId int, 
...
Select 
	...
	case when rb.ResponseValue in (-4,-9) then 'Invalid Response (No Response)' 
		 when rb.ResponseValue = -8 then 'Invalid Response (Multiple Response)' 
		 when rb.ResponseValue > 999 then ' Invalid Response (Skip Pattern Response)'  
		 else  ISNULL(siLT.Value,rb.ResponseValue) 
	end AS ScaleLabel,
 ...
 *           3.0  12/17/2014 by dgilsdorf: added -4 (appropriately skipped) as an equivalent to -9 (not answered)
 *			
 ******************************************************************************/
AS
BEGIN
	...
	CROSS JOIN
		(SELECT
			...
			SUM(CASE WHEN ResponseValues in (-4,-9) THEN 1 ELSE 0 END) AS wr9,
