...
Dave Gilsdorf
ATL-852
Queue an OAS extract for James to review

-- answers to multiple response questions that should be skipped should still be submitted. 
...
/****** Object:  StoredProcedure [CEM].[PullExportData]    Script Date: 9/22/2016 2:47:04 PM ******/
SET ANSI_NULLS ON
GO
...
begin
	...
	begin
		...
		--set @SQL = 'update #results set '
		--select @sql=@sql + '['+ExportTemplateSectionName+'.'+ExportColumnName+']='''+@notAnsweredCode+''','
		--from #recodes
		--where ExportTemplateColumnID = @etcID
		--and ExportColumnName <> 'unmarked'
		--set @SQL = left(@SQL,len(@SQL)-1)
		--set @SQL = @SQL + ' where len('+@sqlwhere+')=0' + char(10) 
