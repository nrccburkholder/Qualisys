...
select
	right(space(10) + cast(isnull(CGPracticeID,0) as varchar), 10)+		--CGCAHPSPracticeSite_ID
	right(space(10) + cast(isnull(cggroupid,0) as varchar), 10)+					--GroupID
	right(space(50) + rtrim(ltrim(isnull(practicename,''))), 50)+					--PracticeName
    ...
    right('00' + isnull(practiceownership,'  '), 2)+								--Practice Ownership and Affiliation  
	right(space(5) + rtrim(ltrim(isnull(patvisitsweek,0))), 5)+						--Patient Visits Per Week
	right(space(2) + rtrim(ltrim(isnull(provworkweek,0))), 2)+						--Providers Working Each Week
	...
	right(space(30) + rtrim(ltrim(isnull(practicecontactname,''))), 30)+			--Practice Contact Name
	right(space(10) + rtrim(ltrim(isnull(practicecontactphone,''))), 10)+			--Practice Contact Phone
	right(space(30) + rtrim(ltrim(isnull(practicecontactemail,''))), 30)			--Practice Contact Email
from CGCAHPSPracticeSite ps inner join #tmp_data t
 on ps.CGPracticeID = t.sampleunit_id
go