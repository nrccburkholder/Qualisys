...
-- Created: 8/10/04
-- Author:  SJS
-- Descrip: Process scheduledmailing records that have a mailing methodology other than mail, (we won't form or pcl gen them, but will add necessary table entries for a processed record (sentmail/questionform etc.).
-- Modified 05/25/05 SS -- Added LRathje to CC List.
-- Modified 10/09/07 MB -- Added BVaske to CC List.
-- Modified 01/11/07 MB -- changed hardcoded e-mail list to distribution group nonmailgen@nationalresearch.com
-- Modified 09/23/08 MB -- added case logic for sq.language when populating QDEComments table.
-- MODIFIED 11/19/09 MB Added new tables to update (DL_SEL_QSTNS_BySampleSet and DL_SEL_SCLS_BySampleSet).
--                      They will be used in the new Import Results application(s)
-- MODIFIED 03/02/09 MB Added new tables to update (DL_SampleUnitSection_BySampleset).
--                      It will also be used in the new Import Results application(s)
-- MODIFIED 05/14/10 MB Added logic to populate VendorFileCreationQueue table when sampleset is created
-- MODIFIED 07/07/10 MB Added call to QSL_SelectVendorSampleSetFileData to create and save vendor file data
-- MODIFIED 09/02/09 MB Added og.bitIsNonMailGeneration = 1 logic to populate #FG_NonMailingWork
-- MODIFIED 9/19/13 DG Added call to CalcCAHPSSupplemental
------------------  ------------------ ------------------ ------------------ ------------------ ------------------ ------------------ ------------------
ALTER PROCEDURE [dbo].[SP_FG_NonMailGen]
AS
