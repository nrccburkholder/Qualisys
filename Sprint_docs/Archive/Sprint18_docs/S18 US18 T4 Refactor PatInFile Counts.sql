...
Modified: 04/08/10 by Michael Beltz                  
    Modified HHCHAPS PatInfile logic to check in new Qualisys table (CAHPS_PatInFileCounts).  If records in                
...
Modified: 2/6/2015 by Chris Burkholder
	Switched to using CAHPS_PatInfileCount rather than HHCAHPS_PatInfileCount
**************************************************************************************************/                                    
...
if @surveyType_ID = 2                  
    ...
    if @surveyType_ID in (3, 11)
    BEGIN                
        --Check for Patients in File Count.                
        --First loook in CAHPS_PatInfileCount (new way) then if no records are found                
        ...
        set @HDOSLsql = 'insert into #PatInFileDistinctCounts   
                         ...
                         from S' + LTRIM(STR(@Study_id)) + '.Export_' + LTRIM(STR(@ExportSetID)) + CONVERT(VARCHAR(40), @Unique) + ' e,   
                              QUALISYS.QP_PROD.dbo.CAHPS_PatInfileCount p                 
        ...
        BEGIN                
            ...
            if @InDebug = 1         
            begin        
                print 'CAHPS_PatInfileCount returned no results so instead trying...'                
                print @HDOSLsql                        
                Select '#PatInFileDistinctCounts - Sub' [#PatInFileDistinctCounts], * from #PatInFileDistinctCounts        
            end        
