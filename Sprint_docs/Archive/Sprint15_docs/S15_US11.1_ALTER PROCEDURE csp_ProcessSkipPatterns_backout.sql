...
ALTER PROCEDURE [dbo].[csp_ProcessSkipPatterns] @ExtractFileID INT 
...
AS
	SET NOCOUNT ON
		...
		----,qf.*,s.*
		UPDATE bt
		SET responseVal = CASE WHEN q38694RV.responseVal <> 1 AND bt.[responseVal] NOT IN (-8,-9) AND bt.[responseVal] < 10000 AND bt.[nrcQuestionCore] <> 38694 THEN bt.[responseVal] + 10000 
							   WHEN q38726RV.responseVal <> 1 AND bt.[nrcQuestionCore] = 38727 AND bt.[responseVal] NOT IN (-8,-9) AND bt.[responseVal] < 10000 THEN bt.[responseVal] + 10000 
							   ELSE bt.[responseVal] 
					      END
