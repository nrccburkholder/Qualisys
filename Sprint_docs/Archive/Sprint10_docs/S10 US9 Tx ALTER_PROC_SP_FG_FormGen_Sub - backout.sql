...
--     AND (mw.bitSendSurvey=1 OR sus.SelQstnsSection=-1)
 ...
 SELECT mw.ScheduledMailing_id, mw.SamplePop_id, mw.Survey_id, MAX(ss.SampleUnit_id) SampleUnit_id,
     sus.SelQstnsSection AS Section_id, 0 AS Langid, mw.bitMockup, mw.MailingStep_id
 INTO #PopSection
 FROM #FG_MailingWork mw, dbo.SelectedSample ss, SampleUnitSection sus
 WHERE mw.SampleSet_id=ss.SampleSet_id
    AND mw.Study_id=ss.Study_id
 ...
 GROUP BY mw.ScheduledMailing_id, mw.SamplePop_id, mw.Survey_id,
     sus.SelQstnsSection, mw.bitMockup, mw.MailingStep_id
...
-- remove records from #PopSection if the section is mapped to a mode other than what is currently being generated.
 ...
 FROM #PopSection ps
 INNER JOIN MailingStep ms on ps.MailingStep_id=ms.MailingStep_id
 INNER JOIN ModeSectionMapping msm on ps.Survey_id=msm.Survey_id and ps.Section_id=msm.Section_id
