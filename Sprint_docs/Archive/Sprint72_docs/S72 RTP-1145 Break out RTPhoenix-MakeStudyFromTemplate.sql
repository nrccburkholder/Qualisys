...
begin try
	...
	--check for existing study of same name in target client

	declare @targetStudyName char(10) = @Study_NM
	if @targetStudyName is null
		select @targetStudyName = [STRSTUDY_NM]
			from [RTPhoenix].[StudyTemplate] 
			where study_id = @study_id

	if exists(select 1 from [dbo].[study] db0
		where db0.client_id = @TargetClient_ID 
			and db0.STRSTUDY_NM = @targetStudyName)
	begin
		INSERT INTO [RTPhoenix].[TemplateLog]([TemplateID], [TemplateJobID], [TemplateLogEntryTypeID], [Message] ,[LoggedBy] ,[LoggedAt])
			 VALUES (@Template_ID, @TemplateJob_ID, @TemplateLogEntryError, RTrim(convert(nvarchar,@targetStudyName)) + ' study already exists for Client_ID: '+convert(varchar,@TargetClient_ID), @user, GetDate())
		
		UPDATE [RTPhoenix].[TemplateJob]
		   SET [CompletedNotes] = RTrim(convert(nvarchar,@targetStudyName)) + ' study already exists for Client_ID: '+convert(varchar,@TargetClient_ID)
			  ,[CompletedAt] = GetDate()
		 WHERE [TemplateJobID] = @TemplateJob_ID
...
begin catch
	...
	*/
	...
	--For each survey name in the template
	...
	----Else generate from Template (using left join to TemplateJob where null)
		...
		SELECT 2--[TemplateJobTypeID]
		  ...
		  LEFT JOIN [RTPhoenix].[TemplateJob] tj2 
					on tj2.[MasterTemplateJobID] = @TemplateJob_ID
					and tj2.[TemplateJobTypeID] = 2 
					and sd.STRSURVEY_NM = tj2.[SurveyName]
		  WHERE tj.[TemplateJobID] = @TemplateJob_ID
					and tj2.TemplateJobID is null --means no existing template job exists for this survey name
	end
	else
