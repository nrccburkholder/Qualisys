...
--@Template_ID must be assigned here if it is not already set because MakeSurvey may be the first called
	...
	--check for existing survey of same name in target

	declare @targetSurveyName char(10) = @Survey_NM
	if @targetSurveyName is not null
	begin
		if exists(select 1 from [dbo].[survey_def] db0
				where db0.study_id = @TargetStudy_ID
					and db0.STRSURVEY_NM = @targetSurveyName)
		begin
			INSERT INTO [RTPhoenix].[TemplateLog]([TemplateID], [TemplateJobID], [TemplateLogEntryTypeID], [Message] ,[LoggedBy] ,[LoggedAt])
				 VALUES (@Template_ID, @TemplateJob_ID, @TemplateLogEntryError, 
					RTrim(convert(nvarchar,@targetSurveyName)) + ' survey already exists for Study_ID: '+convert(varchar,@TargetStudy_ID), @user, GetDate())
		
			UPDATE [RTPhoenix].[TemplateJob]
			   SET [CompletedNotes] = RTrim(convert(nvarchar,@targetSurveyName)) + ' survey already exists for Study_ID: '+convert(varchar,@TargetStudy_ID)
				  ,[CompletedAt] = GetDate()
			 WHERE [TemplateJobID] = @TemplateJob_ID
	...
	else --indicates @targetSurveyName is null
	if exists(select 1 from [RTPhoenix].[SURVEY_DEFTemplate] sd inner join
					[dbo].[Survey_Def] db0 on sd.strsurvey_nm = db0.strsurvey_nm 
				WHERE db0.study_id = @TargetStudy_id and sd.study_id = @study_id)
	begin
		INSERT INTO [RTPhoenix].[TemplateLog]([TemplateID], [TemplateJobID], [TemplateLogEntryTypeID], [Message] ,[LoggedBy] ,[LoggedAt])
			 VALUES (@Template_ID, @TemplateJob_ID, @TemplateLogEntryError, 
				'One or more template survey names already exist for target Study_ID: '+convert(varchar,@TargetStudy_ID), @user, GetDate())
		
		UPDATE [RTPhoenix].[TemplateJob]
		   SET [CompletedNotes] = 'One or more template survey names already exist for target Study_ID: '+convert(varchar,@TargetStudy_ID)
			  ,[CompletedAt] = GetDate()
		 WHERE [TemplateJobID] = @TemplateJob_ID
