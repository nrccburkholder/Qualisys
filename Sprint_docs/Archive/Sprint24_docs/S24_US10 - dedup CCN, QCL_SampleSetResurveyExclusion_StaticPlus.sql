select *
from sys.procedures
where object_id in (select id from syscomments where text like '%sp_Samp_IdentifyDataSets%')
order by 1

sp_helptext sp_Samp_CreateTempTables	
sp_helptext sp_Samp_IdentifyUnitUniverse
sp_helptext sp_Samp_IdentifyUniverse
sp_helptext sp_Samp_FormatUnivSQL
sp_helptext sp_Samp_IdentifyDataSets

exec sp_helptext sp_Samp_IdentifyUniverse
exec sp_helptext sp_Samp_IdentifyUniverse2
exec sp_helptext sp_Samp_Populate_Sampleunit_Universe
...
*/
CREATE PROCEDURE [dbo].[sp_helptext QCL_SampleSetResurveyExclusion_StaticPlus]
  @Study_id                      INT,
  @Survey_ID                     INT,
...
AS
          ...
          FROM   (
                 ...
                 SELECT t.Pop_id,
                  ...
                  WHERE  t.Pop_id = ss.Pop_id
                         ...
                         AND ss.sampleEncounterDate BETWEEN @minDate AND @maxDate) a
          GROUP  BY a.Pop_id,
  ...
  SELECT DISTINCT '
                    ...
                    + ',9999999)'
                    + '                        
  FROM dbo.SelectedSample ss, dbo.SampleUnit su, S'
                    + LTRIM(STR(@Study_id))
                    + '.Population p                    
  WHERE ss.Study_id='
                    + LTRIM(STR(@Study_id))
  ...
  '''
                    ...
                    + '''                     
  AND ss.Pop_id=p.Pop_id AND  ss.sampleunit_id= su.sampleunit_Id and  su.bitHCAHPS = 1                
...
IF @country=''US''
  ...
  --select *          
  --into dbo.mb_Sampling_HHdistinct_'
...
IF @country=''US''
  ...
  SELECT DISTINCT '
                    ...
                    + ',9999999)'
                    + '                        
  FROM dbo.SelectedSample ss, dbo.SampleSet sset, S'
                    + LTRIM(STR(@Study_id))
                    + '.Population p                    
  WHERE ss.Study_id='
                    + LTRIM(STR(@Study_id))
  ...
  AND sset.datSampleCreate_dt>'''
                    + CONVERT(VARCHAR, DATEADD(DAY, -@ReSurvey_Excl_Period, GETDATE()))
                    + '''                    
