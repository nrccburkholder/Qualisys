/*
	...
	S68 ATL-1396 Fix OAS Sampling for Two Encs on Same Day

	As an OAS CAHPS vendor, if a patient has two encounters on the same day we only want to sample one, 
	so that we are in compliance with protocols.

	NOTES:
	ATL-752 updated SelectEncounterUnitEligibility so that we selected the most recent encounter, 
	based on max encounter date. But if there are two encounters with the same date, it is keeping both 
	in the sample frame and can sample both. Need to randomly pick one to sample.

	ACCEPTANCE CRITERIA:
	If a patient has two encounters on the same day, only one is sampled.
	If a patient has two encounters on different days, the most recent is sampled (regression).


	Tim Butler

	ALTER PROCEDURE [dbo].[QCL_SelectEncounterUnitEligibility]OLL

*/

USE [QP_Prod]
GO
/****** Object:  StoredProcedure [dbo].[QCL_SelectEncounterUnitEligibility]    Script Date: 2/2/2017 10:14:21 AM ******/
SET ANSI_NULLS ON
GO
