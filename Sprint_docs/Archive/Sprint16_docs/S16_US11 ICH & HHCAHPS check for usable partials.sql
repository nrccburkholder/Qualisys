...
Other survey types are using unusedreturn_id=5, so I added "Surveytype_id=10" to the initial query in [CheckForACOCAHPSUsablePartials]
and added similar functionality for HHCAHPS and ICHCAHPS Surveys in [csp_GetQuestionFormExtractData].
...
drop table #partials
go
if exists (select * from sys.procedures where schema_id=1 and name='CheckForMostCompleteUsablePartials')
...
--
 --   declare @ExtractFileID int
	...
	BEGIN
		...
		DECLARE @oExtractRunLogID2 INT;
		DECLARE @currDateTime21 DATETIME = GETDATE();
		DECLARE @currDateTime22 DATETIME;
		DECLARE @TaskName2 VARCHAR(200) =  OBJECT_NAME(@@PROCID);
		SET @TaskName2 = @TaskName2 + ' - CAHPS Incompletes'
		EXEC [InsertExtractRunLog] @ExtractFileID, @TaskName2, @currDateTime21, @ExtractRunLogID = @oExtractRunLogID2 OUTPUT;
		...
		SET @currDateTime22 = GETDATE();
		SELECT @oExtractRunLogID2,@currDateTime22,@TaskName
		EXEC [UpdateExtractRunLog] @oExtractRunLogID2, @currDateTime22
	END
