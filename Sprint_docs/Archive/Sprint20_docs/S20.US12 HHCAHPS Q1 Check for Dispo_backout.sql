...
use qp_prod
go
if exists (select * from SurveyTypeDispositions where SurveyType_ID=3 and Disposition_ID in (25,26))
	delete from SurveyTypeDispositions where SurveyType_ID=3 and Disposition_ID in (25,26)
go
if exists (select * from datamart.qp_comments.dbo.SurveyTypeDispositions where SurveyType_ID=3 and Disposition_ID in (25,26))
	delete from datamart.qp_comments.dbo.SurveyTypeDispositions where SurveyType_ID=3 and Disposition_ID in (25,26)
	
go
if exists (select * from sys.procedures where name = 'HHCAHPSCompleteness' and schema_id=1)
	drop procedure dbo.HHCAHPSCompleteness
go
if not exists (select * from sys.objects where name = 'HHCAHPSCompleteness' and schema_id=1 and type='FN')
exec ('CREATE FUNCTION dbo.HHCAHPSCompleteness (@QuestionForm_id INT)  
RETURNS BIT  
AS  
BEGIN  
