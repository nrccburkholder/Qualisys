...
where etc.SourceColumnName='OriginalQuestionCore in (54067,55137)'
...
update etcr 
set responselabel = 'Some other language'
from CEM.ExportTemplateColumnResponse etcr
where etcr.responselabel='Some other language (qstncore=54067)/Vietnamese (qstncore=55137)'
and etcr.rawvalue = 6
and ExportTemplateColumnID in (select ExportTemplateColumnID
								from CEM.ExportTemplate_view
								where ValidEndDate>'1/1/16'
...
where ExportTemplateColumnResponseID in (select ExportTemplateColumnResponseID
				from CEM.ExportTemplate_view
				where ValidEndDate>'1/1/16'
				and  exporttemplatename = 'cahps hospice'
				and (SourceColumnName like '%54067%' or SourceColumnName like '%55137%')
