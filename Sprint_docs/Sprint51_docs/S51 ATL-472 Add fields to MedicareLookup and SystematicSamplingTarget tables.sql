...
IF NOT EXISTS(select * from QUALPRO_PARAMS where strparam_nm = 'SurveyRule: BypassInitRespRateNumericEnforcement - OAS CAHPS')
INSERT INTO QUALPRO_PARAMS (STRPARAM_NM, STRPARAM_GRP, STRPARAM_TYPE, STRPARAM_VALUE, COMMENTS)
VALUES ('SurveyRule: BypassInitRespRateNumericEnforcement - OAS CAHPS','SurveyRules','S','1','Bypass Init Resp Rate Numeric Enforcement for OASCAHPS')
GO

IF NOT EXISTS(select * from QUALPRO_PARAMS where strparam_nm = 'SurveyRule: CompliesWithSwitchToPropSamplingDate - OAS CAHPS')
INSERT INTO QUALPRO_PARAMS (STRPARAM_NM, STRPARAM_GRP, STRPARAM_TYPE, STRPARAM_VALUE, COMMENTS)
VALUES ('SurveyRule: CompliesWithSwitchToPropSamplingDate - OAS CAHPS','SurveyRules','S','1','Complies With Switch To Prop Sampling Date for OASCAHPS')
GO
 ...
 DECLARE @MailedBeyond42Days TABLE (sampleunit_id int, intsampled int)    
  ...
  --b/c HCAHPS unit will now have an numInitResponseRate of 0 (causes division by 0 error below)    
   INSERT INTO #SampleUnit_Temp    
    SELECT SampleUnit_id, @SamplesLeft, intTargetReturn, numResponseRate, case when (numInitResponseRate = 0 and (su.bitHCAHPS = 1 or su.CAHPSType_id = 16)) then dbo.GetHCAHPSEstResponseRate(su.sampleunit_ID) else numInitResponseRate end    
