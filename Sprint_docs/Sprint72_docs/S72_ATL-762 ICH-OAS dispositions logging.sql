/*
    S72_ATL-762 ICH-OAS dispositions logging.sql

	Chris Burkholder

	3/31/2017

	ALTER VIEW DISPOSITIONS_VIEW
...
Select	d.* 
		...
		,oas.Value OASCAHPSValue
		,oas.Hierarchy OASCAHPSHierarchy
		,oas.[Desc] OASCAHPSDesc
...
from Disposition d 
		...
		left outer join SurveyTypeDispositions aco on d.Disposition_id = aco.Disposition_ID and aco.SurveyType_ID in (10,13)
		left outer join SurveyTypeDispositions ich on d.Disposition_id = ich.Disposition_ID and ich.SurveyType_ID = 8
		left outer join SurveyTypeDispositions oas on d.Disposition_id = oas.Disposition_ID and oas.SurveyType_ID = 16
...
/*************************************************************************************************/
    ...
    WHERE  cqw.QuestionForm_id = qf.QuestionForm_id 
           AND qf.SentMail_id = scm.SentMail_id 
           AND dv.ichcahpsvalue = cqw.finalDisposition 
           AND cqw.Surveytype_id = st.SurveyType_ID
		   and st.SurveyType_dsc in ('ICHCAHPS')
    ...
    WHERE  cqw.QuestionForm_id = qf.QuestionForm_id 
           AND qf.SentMail_id = scm.SentMail_id 
           AND dv.oascahpsvalue = cqw.finalDisposition 
           AND cqw.Surveytype_id = st.SurveyType_ID
		   and st.SurveyType_dsc in ('OAS CAHPS')
