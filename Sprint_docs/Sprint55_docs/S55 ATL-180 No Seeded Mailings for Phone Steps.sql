...
--Tim Butler 08/03/2016 - S55 ATL-180 Added modification to blank out phone number in population table for seedmailingreceiver. 
--Tim Butler 08/10/2016 - Discovered that this was only working for HCAHPS, HHCAHP and ACOCAHPS.  Modified to use CAHPSType_id when looking for SampleUnit
ALTER PROCEDURE [dbo].[QCL_PopulateSeedMailingInfo]
	@sampleset_id int
as
...
declare @surveytype_id int --08/10/2016 TSB
declare @isSeededMailing bit --08/10/2016 TSB
...
select @isSeededMailing = SeedMailings --08/10/2016 TSB
from surveytype st inner join survey_def sd
 on st.surveytype_id = sd.surveytype_id
where sd.survey_id = @survey_id

select @surveytype_id = SurveyType_id --08/10/2016 TSB
from survey_def sd
where sd.survey_id = @survey_id


--If seed unit field exists, then find an appropriate sampleunit_id to copy data from
if @isSeededMailing = 1
begin
	...
	select @sql = 'select top 1 sampleunit_id into tmp_seededsampleunit from sampleunit where sampleplan_id = ' + cast(@sampleplan_id as varchar) + ' and CAHPSType_id = ' + CONVERT(varchar,@surveytype_id) --08/10/2016 TSB
...
-- S55 ATL-180 08/03/2016
DECLARE @schema varchar(10)
SET @schema = 'S' + CAST(@study_id as varchar)
-- check to see if the phone column exists in the population table for this study
IF EXISTS (select 1
	from QP_PROD.information_schema.columns c
	where table_schema = @schema and table_name = 'population' and
			column_name = 'phone')
BEGIN
    -- it exists, so blank out phone on our seed row in the population table. 
	select @sql = 'update s' + @study + '.population set phone = NULL where pop_id = ' + cast(@receiverpop_id as varchar)
	exec (@sql)
END
...
-- Delete records for seeded recipients for phone  S55 ATL-180
DELETE t
FROM #temp t
WHERE Pop_id < 0 -- seeded recipients will have a negative pop_id
and MailingStepMethod_id <> 0 -- excluding all but mail
	...
	begin
		...
		begin
			...
			begin
				...
				-- 2/26/2015 DG Emergency Change - assembling @sql_join should happen outside the IF/THEN that adds TB_# and Art_#, otherwise @sql_join='' the second time through the loop
				if charindex('and isnull(sp.tb_'+@i+',0)=isnull(cv.tb_'+@i+',0)',@sql_join) = 0
...
IF @bitTP = 0 
  BEGIN 
	...
	FROM #FG_MailingWork
      ...
      -- Only want to schedule the next ungenerated record, excluding seeded recipients on non-mail steps   
      ...
      FROM   #fg_mailingwork mw 
             ...
             LEFT OUTER JOIN #fg_mailingwork mw2  ON mw.nextmailingstep_id = mw2.mailingstep_id AND mw.samplepop_id = mw2.samplepop_id 
