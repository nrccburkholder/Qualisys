...
IF OBJECT_ID('tempdb..#CatalystWork') IS NOT NULL DROP TABLE #CatalystWork
IF OBJECT_ID('tempdb..#lt') IS NOT NULL DROP TABLE #lt  

 DECLARE @Study_ID int , @DataFile_id int, @DRGOption varchar(20) = 'DRG'
...
CREATE TABLE #Work (DRG varchar(3), HServiceType varchar(42), HVisitType varchar(42), HAdmissionSource varchar(42), HDischargeStatus varchar(42), HAdmitAge int, HCatAge varchar(42), Enc_id int, EncounterDateField varchar(20), isPastSubmission bit,  DischargeDate datetime, ServiceDate datetime)                      
...
----DECLARE @EncounterDateField varchar(20)

------ find the field field used for the sampling encounter date
--SELECT * --@EncounterDateField = f.STRFIELD_NM                   
--FROM Metatable t
--INNER JOIN Metastructure s on t.table_id = s.table_id
--INNER JOIN metafield f on f.field_id = s.field_id                  
--WHERE t.strTable_nm = 'Encounter' 
--and t.study_id = @Study_ID   
--and (f.STRFIELD_NM = 'DischargeDate' or f.STRFIELD_NM = 'ServiceDate')                   
------SET @MCond = SUBSTRING(@MCond, 1, LEN(@MCond)-3)
...
update #work
	SET DischargeDate = '2016-02-01 00:00:00.000'
where DRG = 876  

update #work
	SET DischargeDate = NULL,
	ServiceDate = '2016-02-01 00:00:00.000'
where DRG = 999 

update #work
	SET DischargeDate = '2015-02-01 00:00:00.000'
where DRG = 789 
...
update #catalystwork
	SET ServiceDate = '2015-02-01 00:00:00.000'
where Enc_id in (789)  
...
-- ****************************** Check to see if #Work has records that are past the submission date ************************    
	...
	'AND sub.SubmissionDateClose < ''' + CAST(@MyDate as varchar) + ''''
...
BEGIN
	...
	BEGIN
		...
		INSERT INTO #Log (RecordType, RecordsValue) Select @DRGOption +': Records not applied because DischargeDate is past the submission. (Study also contains a ServiceDate field, which was ignored):',  LTRIM(STR(@myRowCount))
	END
