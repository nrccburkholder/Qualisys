...
/****** Object:  StoredProcedure [dbo].[QCL_InsertHCAHPSDQRules]    Script Date: 11/16/2006 13:05:05 ******/
IF  EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[QCL_InsertHCAHPSDQRules]') AND OBJECTPROPERTY(id,N'IsProcedure') = 1)
...
BEGIN
	...
	EXEC dbo.QCL_InsertDefaultCriteriaClause 1, @CriteriaStmt_id, @Table_id, 'ADDRERR', '=', 'E501', '', @CriteriaClause_id OUTPUT
END
GO

/****** Object:  StoredProcedure [dbo].[SV_HCAHPS]    Script Date: 11/17/2006 09:45:12 ******/
IF  EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[SV_HCAHPS]') AND OBJECTPROPERTY(id,N'IsProcedure') = 1)
    DROP PROCEDURE [dbo].[SV_HCAHPS]
GO
