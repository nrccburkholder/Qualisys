...
DECLARE @minDate DATETIME, @maxDate DATETIME, @sql VARCHAR(8000)  
...
BEGIN  
 ...
 UPDATE #SampleUnit_Universe SET HouseHold_id=id_Num WHERE HouseHold_id IS NULL

 CREATE INDEX tmpIndex2 ON #SampleUnit_Universe (HouseHold_id)
 ...
 UPDATE #SampleUnit_Universe SET HouseHold_id=NULL WHERE HouseHold_id=id_Num

END  

--Now to expand to people not in this sample  
IF @HouseHoldingType='A' AND @ReSurveyMethod_id=2  
BEGIN      
 ...
 'x.',',9999999),ISNULL('),', ,',','),12,2000)+',9999999)'+'      
 FROM SelectedSample ss, S'+LTRIM(STR(@Study_id))+'.Population p  
 WHERE ss.Study_id='+LTRIM(STR(@Study_id))+'  
 AND sampleEncounterDate BETWEEN '''+CONVERT(VARCHAR,@minDate)+''' AND   
...
ELSE IF @HouseHoldingType='A' AND @ReSurveyMethod_id=1  
BEGIN      
 SELECT @sql='CREATE TABLE #Distinct (a INT IDENTITY(-1,-1), '+@strHouseHoldField_CreateTable+')      
 INSERT INTO #Distinct ('+@strHouseHoldField_Select+')      
 SELECT DISTINCT '+SUBSTRING(REPLACE(REPLACE(REPLACE(@strHouseHoldField_Select,'POPULATION',''),  
 'x.',',9999999),ISNULL('),', ,',','),12,2000)+',9999999)'+'      
 FROM SelectedSample ss, SampleSet sset, S'+LTRIM(STR(@Study_id))+'.Population p  
 WHERE ss.Study_id='+LTRIM(STR(@Study_id))+'  
